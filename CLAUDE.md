...
- **The rule-lifecycle board is five independent signals, and a firing signal files a capture rather than standing as a notice.** Run `python resources/rule_signals.py .` — it computes MEASURED (rule-statement count against the ceiling), AUDITED (a sweep is due), BORN (missing gate dispositions), MAINTAINED (near-duplicate rules) and REPEALED (live rules naming a retired term). Nothing is stored; the board is recomputed from the artifacts every time, because a state file must be maintained and the first session that forgets makes the board lie.
