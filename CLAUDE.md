...
- **The rule-lifecycle board is six entries in two classes — four signals and two reports — and a firing signal files a capture rather than standing as a notice.** Run `python resources/rule_signals.py .`.

  **The four SIGNALS have a trigger and can fire:** BORN (missing gate dispositions), CONTRADICTED (a commit where every gate disposition says the gate was not needed while the always-loaded rule-statement count rose), MAINTAINED (near-duplicate rules) and REPEALED (live rules naming a retired term).

  **The two REPORTS measure and never fire:** MEASURED (rule-statement growth) and AUDITED (whether a three-lens sweep is due). Both lost their trigger when the rule-corpus ceiling was removed, and no replacement threshold is defensible — inventing one is the bare-number failure this project bans.

  **The header counts signals only, and the distinction is load-bearing rather than cosmetic.** A denominator including the reports invites the reading that six things are being watched when four are, and this project's own standard — written about this very board — is that a check which over-claims makes the corpus look guarded when it is only partly guarded. Keep the classes separate as entries are added, so the count stays meaningful.

  Nothing is stored; the board is recomputed from the artifacts every time, because a state file must be maintained and the first session that forgets makes the board lie.
