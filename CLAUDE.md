...
- **Author method text 4.8-shaped.** Every self-hosting build batch and spec-writing batch is run against [`resources/4-8-authoring-heuristic.md`](resources/4-8-authoring-heuristic.md) before its authored text ships — the short checklist distilling what Opus 4.8 (this project's model target) actually steers on. Self-hosting scope: the heuristic doc is host-only and not in the plugin package, so the check stays in this project. Revisit shipping the check if the heuristic doc ever ships into the package.
...
A goal session runs with the plugin turned off so Claude works autonomously through several build batches in one chat, closed by a manual /done. This is the developer's autonomous-build workflow; cruise control is the consumer-facing version that will grow from it. The shape below is defined — run it this way rather than re-improvising it each time.

**The run.** Claude works the batches back-to-back in one session, plugin off, owning the sequencing itself and stopping only for what genuinely needs the user. It uses a single aggregate `_build.md` that lists the batches it will work through, kept purely as a working-state and resume record. With the plugin off the scope-lock hook does not fire, so this aggregate `_build.md` is for state, not enforcement: it is what a resumed session reads to recover where the run was, not a guardrail on which files may change.
...
- It does the LOG-hash backfill by hand, because the session-start hook never fired this session so its automatic backfill did not run. Fill any `[HASH]` placeholders in `LOG/` using the Push step's rule — replace the token only in hash position (an entry heading or the start of an index line), resolve each to the **oldest** `git log -S "<entry title>"` match — and fold the edit into the run's commit. The current session's own entries can't be filled until their commit exists; the next session's hook, or the Push step's backstop, fills those.
...
**Status:** Target v1.13.0. Repo on GitHub, method docs set up (/setup complete).
...
- SPEC.md is a normal doc, changed only through a planned spec-edit batch that /next executes — not edited inline during /plan, and not touched by a feature build. The scope-lock denies SPEC unless the active batch lists it in Files, so only a spec-edit batch (which lists SPEC.md) can edit it.
