...
- **A session that authored or amended a rule in the method's own text cannot close until its LOG entry carries a `Rule gate:` line.** The gate was run and what it decided, or it was not needed and why:
  ...
  **The trigger is mechanical, and stronger than FAQ-sync's** — a commit touching `plugin/throughliner/docs/`, `resources/self-authoring-rules.md`, `resources/rule-maintenance.md`, `resources/method-compliance-audit-checklist.md` or this file. It reaches files in this repository only, so a rule living outside it — a folder-level CLAUDE.md above the project — gets no gate, said plainly rather than implied covered. FAQ-sync's own trigger ("is this change user-facing?") cannot be detected at all and has to ride a close-time read; this one is visible from git with no judgment involved.
