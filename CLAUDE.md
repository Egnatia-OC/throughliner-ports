...
- **The gate's site is /plan's keep-step, and the disposition is written into the queue item there. /next TRANSCRIBES what the item already carries; the close transcribes it again into the LOG entry. Neither composes one.**
  ...
  **A build that finds itself authoring a rule halts only where no disposition exists in the build view or on the item; where the view carries one, the build transcribes it** — that is the one thing /next does with the gate beyond copying. The view pulls each cleared item's labelled `Rule gate:` line into that item's block, so the ordinary case is transcription, and the halt is for a disposition that genuinely was never written.
...
- **A session that authored or amended a rule in the method's own text cannot close until its LOG entry carries a `Rule gate:` line.** The gate was run and what it decided, or it was not needed and why:
  ...
  **This is the FAQ-sync rule's shape extended to a second subject, not a new obligation** — which is why it is authored as an amendment and consumes no slot. It is copied from FAQ-sync deliberately: a required artifact turns a silent omission into a visible one. "Not needed because X" is a claim a later reader can disagree with, and a missing line is a gap anyone can see. A pointer makes the gate visible; only this makes it produce evidence either way.

  **The trigger is mechanical, and stronger than FAQ-sync's** — a commit touching `plugin/throughliner/docs/`, `resources/self-authoring-rules.md`, `resources/rule-maintenance.md` or this file. FAQ-sync's own trigger ("is this change user-facing?") cannot be detected at all and has to ride a close-time read; this one is visible from git with no judgment involved.
