...
- **A session that authored or amended a rule in the method's own text cannot close until its LOG entry carries a `Rule gate:` line.** The gate was run and what it decided, or it was not needed and why:
  ...
  **Nothing in the hooks or the shipped docs changes for this, and that is a finding rather than an omission.** The obligation lives in this always-loaded file, so both the build and the close read it without any change to `docs-b/`. Consumers never author method rules, so shipping it would put a rule in front of people it can never fire for. The build working file is written by /next, but the *content* it carries here is host-only, and the scope-lock already lets a session write its own working file. `resources/rule_signals.py` is untouched too: BORN and CONTRADICTED read the LOG entry, and the LOG entry still carries the line. **Neither gains any ability to tell an honest disposition from a dishonest one, and this change must not be described as fixing that.**
