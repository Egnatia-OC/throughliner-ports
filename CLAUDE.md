...
- **A session that authored or amended a rule in the method's own text cannot close until its LOG entry carries a `Rule gate:` line.** The gate was run and what it decided, or it was not needed and why:
  ...
  **It over-fires, and that is the design rather than a defect.** Touching `docs/` for a typo is not authoring a rule, but a false fire costs one line — "not needed, typo fix". FAQ-sync makes exactly this trade and it is why it works.
