...
- **A session that authored or amended a rule in the method's own text cannot close until its LOG entry carries a `Rule gate:` line.** The gate was run and what it decided, or it was not needed and why:
  ...
  **The trigger is mechanical, and stronger than FAQ-sync's** — a commit touching `plugin/si-plugin/docs-b/`, `resources/self-authoring-rules.md`, `resources/rule-maintenance.md` or this file. FAQ-sync's own trigger ("is this change user-facing?") cannot be detected at all and has to ride a close-time read; this one is visible from git with no judgment involved.
...
- **Scripting constraints for this repository's own code** — `plugin/si-plugin/scripts/` and `resources/`. Five rules, each from a failure that has already happened here.
