...
- **Run authored method text against the authoring heuristic before it ships.** Every self-hosting build batch and spec-writing batch is checked against [`resources/authoring-heuristic.md`](resources/authoring-heuristic.md). **Its "4.8 — the authoring pass" section is now historical**, not the live target: docset A retired on 2026-08-08 and 4.8 is no longer a supported model, so that checklist describes what the *retired* docset was tuned for. Read it as background on why the corpus reads the way it does, and don't apply it as a standard to new text — the live docset is authored by subtraction for the 5-series, which wants less prescription rather than more. A replacement 5-series authoring pass has not been written; until it is, this rule's live half is the model-agnostic section below. That doc is the home for rule-writing rules (the model-agnostic "Rules about writing rules" section — e.g. when a slipped rule earns a hook vs. just sharper wording), so reach for it both when authoring text and when deciding how to fix a rule that slipped. Self-hosting scope: the doc is host-only and not in the plugin package, so the check stays in this project. Revisit shipping the check if the doc ever ships into the package.
