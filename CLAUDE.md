...
**The method runs on ONE docset, and the target is the 5-series.** Resolved 2026-08-08 by the user's explicit greenlight of [retire-docset-a], superseding the two-docset architecture of 2026-08-02 (which itself superseded the single-4.8 target of 2026-06-15).

The plugin ships `plugin/si-plugin/docs-b/` and nothing else. Every session loads it — there is no model detection, no substitution directive, and no fallback, because there is nothing to choose between. Each skill's SKILL.md names `docs-b/` directly. The folder keeps its `-b` name: renaming it would ripple through every doc, hook and resource for no behavioural gain, so the name is a historical artefact and is fine as one.

**What was retired, and why the record matters more than the change.** Docset A (`docs/`) was the heavy, prescriptive docset the method was built on, carrying the load-bearing why-clauses Opus 4.8 needs to follow a rule reliably. It was frozen on 2026-08-02 as a known-good fallback so a bad migration could never strand the project with no working plugin — a no-strand guarantee the user called non-negotiable at the time. Docset B was authored by *subtraction* from it for the 5-series (Fable 5 and Opus 5), which are fussy the opposite way to 4.8: they over-do — self-verify, expand scope, run verbose — so Anthropic's guidance for them is subtraction rather than more prescription. Research: `resources/research/opus-5-instruction-compliance.md`, `fable-5-instruction-compatibility.md`; A's own steering research, now historical, is `resources/research/opus-4-8-verbosity-steering.md` and `model-instruction-compliance.md`.

**The three conditions, and how each was actually resolved — recorded so the retirement isn't re-litigated as hasty.**

1. **The blitz branch merged.** Verified merged into main before the greenlight.
2. **The register-measurement harness was consciously WAIVED**, in the user's words: *"waive the harness and greenlight it"*. The evidence weighed was that the method runs well live on Fable 5 at low reasoning effort. This permanently closes the measurement window that [docset-b-register-decision-weaker-than-recorded] depended on — that item said its safe window was "while A still stands", and A no longer does. That item and [structural-delimiting-experiment] both need reprocessing in this light.
3. **Nobody is on 4.8.** The community is five people and none runs Opus 4.8, so the regression cost of removing the docset tuned for it lands on nobody.

**What replaces the no-strand guarantee.** Nothing does, and that is the honest statement. With one docset, a missing `docs-b/` in an installed plugin means a session has no procedure docs at all rather than a verbose fallback. `session_start.py` therefore reports that state plainly — naming that it concerns the *installed* copy rather than the working tree — instead of quietly repairing it. A report is weaker than a fallback and is what was traded away.

**The freeze rule is retired with the docset it governed.** It said the freeze barred development but permitted correction, and it recurred because `setup.md` was the one file it excepted. None of that has anything left to apply to. The one part worth keeping as a general lesson: a lighter register is lighter *by subtraction*, and the why-clauses it sheds are load-bearing for whichever model needed them — so registers don't transplant between models, in either direction.

**plugin-behaviour.md stays one file — deliberately (decided 2026-08-02, and unaffected by the retirement).** Anthropic's skill conventions name progressive disclosure; this file keeps a monolithic shape anyway. Don't split it, and don't re-raise this on seeing its length. Progressive disclosure works for material fetched on a trigger — a reference table, a rare procedure. It fails for standing behavioural rules, which steer behaviour that has no trigger to fetch them: a session doesn't know to go and fetch "lead with the decision," it either has it or it doesn't. Most of that file is exactly that kind of rule, so moving it behind an index doesn't defer its cost, it deletes its effect. The size premise was always half-true — the line count reflects hard-wrapping at 80 characters, and the ~500-line figure in Anthropic's guidance is written for SKILL.md, the entry-point file, not a reference doc a skill loads. One argument here *has* now lapsed and is recorded as such: the redirect-compounding argument (that a split would stack an index-fetch on top of the docs-b substitution directive) no longer applies, since that directive is gone. The rest stands on its own.
...
No code method/
  ...
  plugin/                — plugin packaging
    si-plugin/           — target source
      ...
      docs-b/            — procedure docs loaded by skills (the only docset;
                           `docs/` held the retired docset A until 2026-08-08)
...
- **Run authored method text against the authoring heuristic before it ships.** Every self-hosting build batch and spec-writing batch is checked against [`resources/authoring-heuristic.md`](resources/authoring-heuristic.md). The whole doc is live and model-agnostic — three per-text checks, plus the rule-writing guidance on when a slipped rule earns a hook versus just sharper wording. Reach for it both when authoring text and when deciding how to fix a rule that slipped. Self-hosting scope: the doc is host-only and not in the plugin package, so the check stays in this project. Revisit shipping the check if the doc ever ships into the package.
...
- **A test fixture standing in for another program's artifact is built from the real thing at least once.** Where a fixture stands in for something *another program* creates — a directory the installer makes, a file another tool writes, a payload the app sends — build it from the real artifact once, or state in one line inside the test that it encodes an assumption. Otherwise the fixture and the code under test can be built from the same wrong belief, agree with each other, and pass — which is indistinguishable from coverage and actively discourages the check that would find the truth. Bounded deliberately: fixtures exist so tests need not depend on the world, so this fires only for artifacts produced outside our own code, never as a general demand.
...
Work-item ordering in QUEUE.md implicitly assumes the next item sees the previous item's effects. That's true for **target-side** changes — edits to files under `plugin/si-plugin/` that Claude can read at author time. It's false for **host-side** changes — the installed plugin's hooks (`hooks/session_start.py`, `hooks/pre_tool_use.py`), the loaded skill procedure docs (`docs-b/setup.md`, `plan.md`, and the `next*.md` / `done*.md` families), and `docs-b/plugin-behaviour.md` — which only refresh after a reinstall (a rezip, or the reinstall step of a release). A routine push changes nothing the host sees.
...
**branch → builds → soak → differential audit, run through /next as its own queued `[audit]` item over the whole branch span → reconcile (/plan processes the audit's findings, /next builds the repairs it cleared) → merge → branch again.**
...
**The audit is a run, not a chat.** It is a queued `[audit]` work item executed through /next, with its own scope, its own LOG entry and its own report; /plan seeds it, clears it, and afterwards processes what it brings home. It is never performed inline inside a planning session — that shape was tried on 2026-08-08 and produced no visible run, no `[audit]` item and no LOG entry at the time, so the user approved "keep and clear it" believing the audit was being queued when only its repair was. The plan doc's *This audit's queue-item form* section carries the seeded wording.

**Which audit runs at which phase** — three different jobs, and today they have to be reconstructed from three documents:

| Phase | What runs | Trigger | Plan doc |
|---|---|---|---|
| Blitz night | the build run itself, not an audit | the user's greenlight | `resources/overnight-blitz-plan.md` |
| Soak-end, before every merge | **differential** consistency audit | every cycle, as the merge gate | `resources/consistency-audit-plan.md` |
| Big boundaries only | **full-corpus** consistency audit | before a compression pass, after a large redesign, or when differential runs keep finding things | `resources/consistency-audit-plan.md` |
| Its own phase, after a full-corpus audit | **compression pass** | when the docs have grown enough to warrant it; never on the same branch as the audit that precedes it | `resources/compression-pass-plan.md` |

The two consistency modes ask whether the documents agree with each other and with the code. The compression pass asks whether they are longer than they need to be — a different question, which is why it is a separate phase with the full audit as its stated precondition rather than a mode of it.
...
6. git checkout -b <next-branch>         # the "branch again" step — easy to skip,
                                         ...
                                         # straight onto main
6a. SEED THE CYCLE'S THREE STANDING       # see "Seeding the cycle" below — this is
    ITEMS into QUEUE.md                   # what stops every session needing the
                                          # cycle explained to it
7. rezip + FULL app restart              # so sessions run the build being tested
```

#### Seeding the cycle — step 6a, written out

**At rebranch, the new cycle's phases go into the queue as ordinary work items.** Then every session reads the cycle from the queue like any other work, /plan's below-the-line revisit tracks where the cycle stands, and the cycle-status close ask reports from queue state instead of reconstruction. Without this, each phase depends on the user re-explaining the cycle to whichever session is in front of them — which is exactly the working-memory dependence the queue exists to remove.

The three items, all seeded **below the readiness line**, written to be **re-added each cycle rather than re-derived** (the sanctioned repeating-batch shape — no new state, no new marker, no new machinery):

```
#### Run the overnight blitz for this cycle [blitz-run-<cycle>]
Runs `resources/overnight-blitz-plan.md`. Below the line; lift-condition: you
greenlight a blitz night at a /plan close.

#### [audit] Differential consistency audit over this branch's span [differential-audit-<cycle>]
Runs `resources/consistency-audit-plan.md` in differential mode over
`main...HEAD`. Below the line; lift-condition: the branch has stopped taking
new work and the merge is the next thing due. This is the pre-merge gate.

#### [user] Merge this cycle's branch to main and rebranch [merge-<cycle>]
Runs the soak-end sequence above. Blocked by: [differential-audit-<cycle>].
Below the line; lift-condition: the audit has run AND its repair captures have
been cleared. The gate is this item's own prose — a merge item written without
it is the failure the branch-cycle section warns about.
```

**The blitz item is an ordinary build, not `[user]`-flavored, and that was decided rather than defaulted.** The over-tag guard settles it: `[user]` is earned only by work Claude genuinely cannot perform, and Claude can run a blitz. What Claude cannot do is decide that tonight is a blitz night — and that is a readiness question, which the readiness line already expresses. Tagging it `[user]` would be the recorded mistake of confusing "Claude can't do this" with "Claude can't do this yet".

**Seeding is a documented step, not a hook, and that is also deliberate.** The method's own lesson applies directly: a convention honoured in one written place beats a mechanism nobody built, and the retired push marker is the standing evidence for what happens when a gate is documented in several places and implemented in none. If this step proves unreliable in practice, a hook is the escalation, per the authoring heuristic's hook-versus-wording trigger.
...
### The cycle-status ask at every /plan close — with a logged disposition, host-only

The cycle above has phases and a greenlight, and **nothing was ever asking about either.** Several of this queue's items say "greenlight it at any /plan close", and `done-plan.md`'s close carries no such step in either docset — the only raise-point is plan.md Step 1's opening question, which is the wrong shape (a greenlight is a judgment, not an external event that has or hasn't happened) at the wrong end of the session (the opening is where the session holds the least evidence). The result was a user trying across several sessions to greenlight a merge and it never happening, with every individual session behaving correctly. Only the user's own report could have surfaced that.

So: **at every /plan session's close in this project, ask ONE consolidated question** covering both halves —

- **where the branch cycle stands** — building, soaking, audit due, or merge due; and
- **which below-line items are held by the user's choice** rather than by an external event, and are therefore waiting on their word.

Word the ask two-sided, as the stop-self-sufficiency rule requires: **greenlight any of these, or hold them all.** Never a trailing "…anything to greenlight?" that reads as a recommendation to hold.

**The teeth: the session's LOG entry carries a one-line `Cycle:` disposition**, so a close that skipped the ask is visibly missing a required line rather than silently clean:

```
Cycle: soaking, nothing greenlit
Cycle: merge greenlit
Cycle: audit due; [retire-docset-a] greenlit
```

This is deliberately the **FAQ-sync gate's shape** — this file's one demonstrated case of a soft rule growing teeth. A plain convention was rejected as the retired push marker's shape: stated somewhere, performed nowhere. The branch-cycle section above already admits nothing enforces its own gate; this is the one part of the cycle that gets an enforcement mechanism, because it is the part a session can silently skip without any artifact looking wrong.

**Why the close rather than the below-the-line revisit.** The revisit is built for external events — "has it happened yet?" — and answers itself mechanically or with a yes/no. A greenlight is a judgment about whether things have gone well enough, and the close is where the user is holding a session's worth of evidence to make it with.

Host-only by residence, like the FAQ-sync gate and the ripple-grep rules: it lives in this project's CLAUDE.md, which consumer projects don't carry. Consumers have no branch cycle to report on.
...
4. Refresh the installed host from the local-folder marketplace via the `claude` CLI, then fully restart the app. The desktop app no longer has an in-app plugin upload, and a working-tree or zip edit alone changes nothing the installed host sees — the host runs a frozen snapshot the CLI copied into `~/.claude/plugins/cache/...` at install time, not the live files. So testing the new build means re-running the install/update so the CLI re-snapshots the current `plugin/si-plugin`. Claude runs these commands; Alex types nothing in a terminal.
   ...
   **Check the CLI against the app before using it.** Compare `claude --version` (the standalone binary this ritual drives) against the desktop app's version, and say plainly when they differ. They drift silently: on 2026-08-04 the CLI was on 2.1.146 while the app ran 2.1.219, and marketplace features have real version floors (the `renames` map needs 2.1.193 or later), so an old CLI fails in a way that reads as the plugin design being wrong rather than the tool being stale. If it's behind, update it first — `& "$env:USERPROFILE\.local\bin\claude.exe" update` — then carry on.
   ...
   - **Then compare the stamps — target versus installed — before saying anything about the restart.** Run `content_stamp()` over `plugin/si-plugin` and over the cache directory for the version just installed (`~/.claude/plugins/cache/flintcraft/sovereign-implementer/<version>/`), and check they match. Immediately after a reinstall with no edits in between they **must** be identical, so a difference is unambiguous rather than a judgment: either the snapshot didn't take, or the stamp function itself is wrong. Both are worth stopping for. One command:

     ```bash
     python -c "import sys; sys.path.insert(0, 'plugin/si-plugin/hooks'); from session_start import content_stamp; import os; t = content_stamp('plugin/si-plugin'); h = content_stamp(os.path.expanduser('~/.claude/plugins/cache/flintcraft/sovereign-implementer/<version>')); print('target  ', t); print('installed', h); print('MATCH' if t == h and t else 'MISMATCH — stop')"
     ```

     Why this is a step rather than a habit: it caught a real shipped bug on the day it was written down (the `.in_use` exclusion matching a filename when `.in_use` is a directory), and only because the stamp mechanism happened to be what was under test. On any other rezip nobody would have looked. Its honest limit: this proves the installed copy matches the source. It **cannot** tell you the stamp function is computing the right thing — that was the bug underneath, and it was caught only because the two directories were known byte-identical by an independent route (`diff -rq`). A cheap net, not a proof.
...
The model this project mostly runs. Change it by saying so.

**Nothing reads this field any more, and that is deliberate rather than an oversight.** It existed to choose a docset where the payload's `model` field never arrives (the desktop app doesn't send one), and with docset A retired there is no choice to make. It stays because it records what the method is being developed and tested against, which is a real fact about the project even with no mechanism consuming it. Fable 5 at low reasoning effort is also in live use here and is what the README now records as tested.
...
- **The three standing plan docs under `resources/`** — one per phase of the cycle, all host-only dev artifacts (not shipped in the plugin package). `overnight-blitz-plan.md` for a build night; `consistency-audit-plan.md` for both audit modes, and it now owns *all* the audit criteria — its full-corpus passes 11 and 12 are the response-shape-tag-placement and narration-drift lenses folded in from the retired `method-compliance-audit-checklist.md` on 2026-08-08, so there is no longer a third standing audit document to drift; `compression-pass-plan.md` for the pass that edits the docs down. A /plan scoping any of these reuses the doc rather than re-deriving the criteria.
