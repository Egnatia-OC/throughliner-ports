...
**The frontmatter stamp on each `docs-b/` file reads `docset: current`, renamed from `docset: B` on 2026-08-16.** The stamp itself stays and must: session_start's behaviour-rules redirect self-checks against it, so it is the one guard proving the always-loaded rules reached the session at all. What changed is only the value, which used to name a sibling that no longer exists — a self-check asserts a new value exactly as well as an old one.

**The folder keeps the name `docs-b/`, and this is a refusal on the record rather than a deferral.** A rename was traced by grep before being rejected: 639 occurrences across the repository, of which 215 are live paths and **424 sit in `LOG/` and one archived INBOX message**. Renaming does not remove the "B" — it moves it out of a live path, where this file can explain it, and into 424 historical references pointing at a folder that no longer exists. Rewriting those is not available, because the session record is what the whole method rests on and editing it to agree with a later decision falsifies it. Same reasoning that left the deleted Codex port's commits unreachable rather than rewriting history full of hashes the LOG depends on. **So the rename would create drift by fixing drift**, converting a question this file can answer into a permanently unanswerable one.
...
- **Judge a design by whether it makes the intended outcome more likely, not by whether it guarantees it.** A missing guarantee is an objection only where the design depends on that guarantee to work at all. It fires at /plan's keep-step, when weighing whether work is worth doing.
...
- **The gate's site is /plan's keep-step, and the disposition is written into the queue item there. /next TRANSCRIBES what the item already carries; the close transcribes it again into the LOG entry. Neither composes one.**
  ...
  **A build that finds itself authoring a rule with no disposition to transcribe halts and says so** — that is the one thing /next does with the gate beyond copying.

  **When the gate fires at the keep-step, say so in that item's message: what rule is proposed, which parent it amends, and what the gate decided.** One or two sentences folded into a message the user is already reading — never a turn of its own, which is the over-asking this method has spent months removing.
...
- **`resources/rule_signals.py` runs six mechanical checks over the rule corpus, and a check that finds something files a capture rather than standing as a notice.** Run `py resources/rule_signals.py .`. It is for Claude, not for Alex: nothing here asks her to do anything, and its findings reach her as ordinary queue items like any other work.

  **Four of the six can find something:** whether every rule-bearing commit carries a gate line; whether any commit says the gate was not needed while the always-loaded rule text grew; whether two rules say nearly the same thing; and whether a live rule still names a retired mechanism.

  **Two only measure and can never fire:** how much rule text there is, and whether a corpus sweep is due. Both lost their trigger when the rule-corpus ceiling was removed, and no replacement threshold is defensible — inventing one is the bare-number failure this project bans.
  ...
  **The header counts only the four that can fire.** A denominator including the two measurements invites the reading that six things are being watched when four are. Keep the classes separate as entries are added, so the count stays meaningful.
  ...
  **Surface it at a /plan opening**, alongside the digest and the held work. The reason it is not on demand: a refusal leaves no scannable artifact at all — a kept item sits in the queue, a built item leaves a vanished item and a LOG entry named after its slug, and a refused proposal leaves one sentence inside one entry. Asked how she would know to ask for it, the user's answer settled it: **"how would I even know to demand it?"** — which is the siteless-check failure recorded five times here, walked into by the recommendation itself. The window is hers too, tightened from a count of recent entries to *since the last planning session*, because it tracks the moment she actually looks, and it goes quiet by itself when nothing has been refused.
...
- **FAQ entries are part of batch authoring.** When /plan authors a batch that introduces something a consumer would see or ask about — a new queue line, a new doc section, a new narration moment — the batch carries a `plugin/throughliner/templates/faq-template.md` entry (plus its `faq-index-template.md` index line) in its build list. **After writing that entry and its index line, re-copy both files into `FAQ/`** — `FAQ/faq.md` and `FAQ/index.md` are a straight copy of the shipped template, and the template is canonical. Without this clause the rule directs every entry to one file while two exist, so the divergence it produced was generated by the rule itself rather than left behind by an event: 502 lines against 591 on one day, 89 lines of difference where 47 had been measured the day before. `FAQ/` is kept here rather than deleted because `session_start` points every session at `FAQ/index.md` exactly as it does for consumers, and a host that reads its documentation from a different path than its users stops detecting anything about that path. The test is the same one the FAQ-sync gate uses, so both narrow together: does this change alter what a user **does** — a new step, a changed command, something different appearing in their own documents, a moment where they must answer or decide? If yes, and a non-coder meeting it would have a question the FAQ doesn't answer, the FAQ entry ships with the batch. A change that is user-visible but leaves their actions unchanged does not fire it. Host-project rule, not shipped plan.md — consumers never author FAQ entries, so the rule would misfire in their /plan sessions.
...
- **Scripting constraints for this repository's own code** — `plugin/throughliner/scripts/` and `resources/`. Five rules, each from a failure that has already happened here.
  ...
  - **Any subprocess read sets `encoding="utf-8"` explicitly.** Same defect one layer up, and the one a shared module would have missed — `session_start.py` mangled every em-dash until this was added.
...
Work ordering in QUEUE.md implicitly assumes the next item sees the previous item's effects. That's true for **target-side** changes — edits to files under `plugin/throughliner/` that Claude can read at author time. It's false for **host-side** changes — the installed plugin's hooks, the loaded skill procedure docs (`docs-b/setup.md`, `plan.md`, `skill-nonspecific-rules.md`, and the `next*.md` / `done*.md` families) — which only refresh after a rezip or release plus a full app restart.
...
1. **Reset `plugin/throughliner/.claude-plugin/plugin.json` to a clean version**, dropping any `-testN` suffix a rezip left (`1.20.0-test2` → `1.20.0`). Keep the same release-line number — this is not a release bump, it's removing the test marker. Stage the file with the rest of the work.
2. `git push`.
...
**How long `plugin.json` sits dirty, corrected from the working tree rather than reasoned about.** Normally it is dirty only between a rezip and that same session's push. But that holds only if every rezip is followed by a push, and one was not: the file was found carrying `1.20.0-test8` at the opening of a later session, so the window had spanned sessions, and every close in between would have surfaced it as an out-of-scope dirty path. This paragraph used to assert that could not happen.

**So a close meeting a `plugin.json` diff whose only change is the `-testN` suffix recognises it as the known rezip artifact and skips the investigation** — at most a one-line note, never a per-close question. Sibling in shape to the hash-backfill signature skip. Host-only, and it stays here rather than in the shipped `done.md`: consumers never rezip and can never produce that signature.
...
- Design for fresh, short sessions. The system must work for a fresh, short session that carries none of a prior session's memory: the files (SPEC, QUEUE, LOG, the session's build working file) must suffice on their own, and conversation memory is a convenience, never a dependency. Short sessions on the weaker post-Fable development model (from ~2026-06-20) are the design target — and every consumer is already in this case. A long session that remembers everything is the exception, never the case to design for. (This is about robustness to session-memory loss; it does not change the Model target above — 4.8 stays the model the plugin is tuned for.)
