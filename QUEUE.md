...
**Author the 4.8-shaped authoring heuristic and wire it as a standing check** **[4-8-authoring-heuristic]**

Opus 4.8 is the model this plugin targets, yet the plugin keeps steering it with the weak form of brevity instruction — abstract tags and "don't" rules the research shows 4.8 steers on least. Two research files (resources/research/opus-4-8-verbosity-steering.md, model-instruction-compliance.md) name what does steer 4.8: positive quantified targets, a positive exemplar, lead-with-the-decision output structure, explicit scope, and naming the specific verbosity pattern to kill. Those findings sit in prose nobody runs at authoring time. This batch distils them into one short checkable list — an "is this 4.8-shaped?" pass any authored doc, procedure, or SPEC text can be run against before it ships. It lands first because it is the foundation of the verbosity arc: the tag-definition redesign and the standing authoring check both apply this list. Self-hosting scope for now — it shapes how this project's own method text is authored, where every session runs on 4.8. It also wires the doc in as a standing check so it does not become a write-only doc: this project's CLAUDE.md gains an authoring rule running every build and spec-writing batch against the heuristic. The check stays host-only because the doc is host-only — putting it in shipped plan.md would dangle for consumers who don't have it.

Build:
- Create resources/4-8-authoring-heuristic.md: a short checklist distilling the two research files into a runnable authoring pass. Each check positive and concrete, with a one-line good/bad exemplar where it helps. Cover at minimum: quantified length target over abstract adjective; a positive exemplar of the wanted shape; lead with the decision/ask and gate detail behind an explicit request; explicit scope statement (4.8 doesn't generalise an instruction across contexts); name the specific verbosity pattern to kill with its positive replacement; and a not-too-terse guard — concision must come from cutting bloat (meta-narration, restating what was shown, hedging), never from cramming or dropping plain-English explanation a non-coder needs. Keep the list short enough to apply in one pass.
- Head the doc with one line on what it is and when to run it (over any authored doc/procedure/SPEC text, before it ships) and a pointer back to the two source research files. State the aim plainly: anti-overwhelm structure and plain English, never arbitrary terseness. The doc models its own advice — short, positive, exemplified.
- This project's CLAUDE.md (host-only, does not propagate): add a standing authoring rule — every self-hosting build and spec-writing batch is run against resources/4-8-authoring-heuristic.md before its text ships. Self-hosting scope; revisit shipping the check if the heuristic doc ever ships into the package.

Test:
- Self-verifying: the doc exists and each check traces to a finding in one of the two research files.

**Redesign the response-shape tags and add a lead-with-the-decision rule** **[tag-definition-redesign]**
Depends on: [4-8-authoring-heuristic]

The response-shape tags were hardened once (why-clauses, quantified constraints, scope), but the verbosity research finds two gaps that still let 4.8 run long. First, no tag carries a positive exemplar — a one-line model of the output shape it wants — and the research names that the single most effective brevity lever. Second, there is no "lead with the decision, gate the detail" rule anywhere; the research ranks that structural lever just below exemplars, and it is the move that most directly fixes the user's problem — the decision they must act on arrives first, not after a wall of text. This batch adds both. It is the first real application of [4-8-authoring-heuristic], so it depends on that doc and is authored against it. Host-side — plugin-behaviour.md is injected, so the behavioural effect lands after reinstall. Exemplars are kept to one line each so the injected doc grows as little as the compliance bet allows.

Build:
- plugin/si-plugin/docs/plugin-behaviour.md, Response-shape tags: add a one-line positive exemplar to [BRIEF] and [SEQUENCE] — a concrete model of the wanted output shape, not a description of it. One line each.
- plugin/si-plugin/docs/plugin-behaviour.md, Communication: add the lead-with-the-decision structural rule — every user-facing message leads with the one thing the user must see or act on, then stops; detail is gated behind an explicit user request. Positive and quantified, with a good/bad exemplar pair and an explicit scope statement (every skill, every message, including close-outs and walkthroughs).
- Run the [4-8-authoring-heuristic] pass over the new text before finishing.

Test:
- Self-verifying from doc text for structure (exemplars present; rule present with scope).
- Behavioural, host-side (after push + reinstall): later sessions lead with the decision and the brevity tags hold against the wall-of-text pull. Needs the deferred-test discipline — flag at /done if it can't run.

**Ship a concise output style in the plugin package (system-prompt priority)** **[verbosity-output-style]**
Depends on: [4-8-authoring-heuristic]

This is the strongest lever in the verbosity research, and this session's search confirmed it is buildable. The point is not to make Claude terse — it is to take the structural rules the plugin already has and lift them to the priority level where they hold. The plugin's Communication sequencing rule and the [SEQUENCE] tag already say lead with the decision, give one item at a time, gate detail — but they sit at user-message priority, exactly where 4.8 skips them, so the user still gets bundled walls. Output styles modify Claude Code's system prompt directly, reaching the level CLAUDE.md and skill docs cannot. So the style carries the same anti-overwhelm structure, higher. Explicit non-goal: no blanket length cap and no arbitrary terseness — for a non-coder, cramming and dropped explanation defeat the plain-English purpose as badly as a wall of text; plain English is the standard the concision serves. The style sets the structural default for every response; the per-step response-shape tags still govern individual moments, including the [DISCUSS] steps that license length. Shipped in output-styles/ with force-for-plugin: true (auto-applies for every user; tradeoff accepted — overrides a user's own chosen style while SI is enabled) and keep-coding-instructions: true (keeps Claude Code's coding behaviour). Source: resources/research/opus-4-8-verbosity-steering.md; the structural rules live in plugin-behaviour.md Communication + [SEQUENCE].

Build:
- Create plugin/si-plugin/output-styles/<name>.md: an output style that elevates the plugin's existing structural communication rules to system-prompt priority — lead with the one thing the user must see or act on then stop; one item at a time when the user's next action depends on the prior; gate detail behind an explicit request. State the non-goal in the style itself: anti-overwhelm structure and plain English, not terseness or a word-count cap; never cram or drop explanation a non-coder needs. Note per-step depth is governed by the skill tags ([DISCUSS] licenses length). Frontmatter: name, description, keep-coding-instructions: true, force-for-plugin: true. Author with [4-8-authoring-heuristic], consistent with [tag-definition-redesign] and the plugin-behaviour.md sequencing rule.
- Verify at build: confirm whether the plugin auto-discovers output-styles/ or the manifest must register it; wire whatever is needed. Confirm force-for-plugin applies without user selection and layers (keep-coding-instructions) rather than replacing coding behaviour.

Test:
- Self-verifying: the file exists with the four frontmatter fields, carries the structural rules, and states the no-terseness/plain-English standard.
- Behavioural, host-side (after push + reinstall): sessions visibly lead with the decision and chunk one item at a time while staying plain English (not terser/jargon-y); the style shows as forced-active. Needs the deferred-test discipline — flag at /done if it can't run.
