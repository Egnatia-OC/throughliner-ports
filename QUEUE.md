...
**Extend pre_tool_use: deny blanket adds and commit -a; denials teach the fix** **[git-add-safety-hook-gap]**
...
Second fold-in: the Files: bare-path observation from the [hash-backfill-as-hook] build (2026-06-12). A Files: line written with a trailing parenthetical note silently failed to grant scope, because the scope-lock matches Files: lines as exact paths. The deny message made the cause visible only after a failed edit, and nothing documents the bare-path rule. The fix is the move this batch already makes: the denial teaches. The scope-lock deny message gains the rule, and next.md documents it where Files: lines are authored. Tolerant matching of annotated lines was considered and rejected — loosening a safety mechanism to absorb formatting noise is the same trade declined above for the git patterns.
...
- plugin/si-plugin/hooks/pre_tool_use.py scope-lock denial: when an edit is denied because the file isn't in the Files: list, the message states the bare-path rule — Files: lines must be bare paths, one per line, nothing else on the line — alongside the existing allowed-files listing, so an annotation-caused mismatch is self-diagnosing at the moment of denial.
- plugin/si-plugin/docs/next.md, where the _build.md working-file format is defined: state the bare-path rule where Files: lines are authored — bare paths, one per line, nothing else on the line. Locate by content, not line numbers.
...
- Claude-run: synthetic scope-lock payload — a Files: line carrying a trailing annotation fails to grant scope, and the denial names the bare-path rule.
