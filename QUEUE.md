...
**Deferred-test lifecycle: tick state at determination, runnability tails, pre-flight offer, reinstall flag** **[deferred-test-lifecycle]**

From a capture raised at the [git-add-safety-hook-gap] /done (2026-06-12). Deferred tests have no execution trigger: the section makes pending tests visible — /next re-presents them every pre-flight — but nothing tasks anyone with producing the confirming event, so "Confirmed by:" lines describe observations that only happen if someone deliberately acts or happens to notice. [deferred-tests-structural-home] solved surfacing; this solves executing. The user's trigger-flavor observation is the spine: reinstall-gated tests, the dominant flavor in self-hosting, have a mechanically detectable runnable moment, because session_start already reports a version mismatch after a plugin update. Grounded against the current seven-line backlog, the reinstall-gated flavor splits further: deliberately runnable (Claude can produce the event on demand — the scratch git add -A denial, the deliberate why-question run), near-automatic (the event occurs in almost any session — backfill report, lint fire), and observational (confirmable only by watching a session's behaviour). The design announces once and offers persistently: the hook flags the moment, and /next's pre-flight repeats the offer until lines clear, so a missed announcement costs nothing. Runnability is recorded at authoring time as a prose tail, not a closed taxonomy — the same move as [blocked-by-trigger-flavors]. User-run and external-event entries keep re-presentation as their mechanism, per the capture's own judgment. Two folds ride along. From the /clear-resilience discussion this session: _build.md's Progress has no "couldn't run" state, so a mid-session determination that a test can't run lives only in conversation — post-/clear, /done misreads the unticked entry as unfinished work; the fourth tick state writes the determination into the file at the moment it's made. And the capture's request to write the unwritten context: the section's scope statement (test-only, shipped-work-only) currently exists nowhere. session_start.py is shared with [session-start-dirty-tree-check] and [plan-state-artifact] — whichever builds second sees the others' changes, per the standing convention.
...
- plugin/si-plugin/docs/next.md, pre-flight deferred-tests step: extend from re-present to re-present-and-offer — entries whose tail says Claude can run them get an offer to run now; confirmed lines are removed and the confirmation recorded in this session's LOG entry.
- plugin/si-plugin/hooks/session_start.py: when the version-change report fires and QUEUE.md's Deferred tests section is non-empty, add one line to the report — deferred tests may now be live-testable.
...
Test:
- Claude-run: session_start.py against a fixture — version mismatch with a non-empty Deferred tests section adds the line; mismatch with empty section stays silent; no mismatch stays silent.
- Behavioural, host-side (after push + reinstall): the first session after the update shows the hook's deferred-tests line, and the next /next pre-flight offers to run the Claude-runnable backlog — the current seven lines are the natural exercise. Needs the deferred-test discipline — flag at /done if it can't run.
