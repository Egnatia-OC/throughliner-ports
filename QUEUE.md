...
- The hash backfill can corrupt prose that mentions the placeholder token literally. Observed live at the 2f23dc6 backfill: a blanket find-replace over LOG/log.md swapped the token everywhere, including a Files-touched line describing the *previous* backfill — producing "2f23dc6 placeholders backfilled to 98cf4a6", which is gibberish. Fixed by rewording the prose to avoid the literal token. Two implications for [hash-backfill-as-hook]: the hook must replace the token only in hash position (entry headings in log files, line starts in the index), never in body prose; and LOG-entry prose should avoid writing the literal token at all — the same patterns-as-data trap [git-add-safety-hook-gap] documents for the git-safety denials. Until the hook ships, the model-run backfill in plan.md and next.md carries this risk on every run.

- Queue-visible plan markers: when planning work is predictable at authoring time, write it into the queue instead of leaving it to session-end judgment. Observed pattern: Claude keeps recommending "run /plan first" at moments that feel unexpected to the user, but a subset of those moments is predictable when the queue is authored — an audit batch will file findings the next batch depends on; a batch's Depends on: can only be estimated after a design decision a planning session has yet to make. Proposal: an inter-batch marker line, sibling to the push-marker convention — e.g. "--- Plan session here: <reason> ---" — that /next's pre-flight halts on. Framed as a floor, not a ceiling: planning must happen here at minimum, for the stated reason; ad-hoc /plan elsewhere stays unrestricted. Converts a judgment-based recommendation into a mechanical gate (weak-model friendly, the same bet as the push marker), makes the planning rhythm visible in the queue before it's hit, and carries the reason inline. Interplays: done-audit already defaults to recommending /plan after audits — this makes that queue-visible before the audit runs; related to but distinct from the clarity-shaped triggers in [blocked-by-trigger-flavors], which cover parked-item trigger semantics rather than active-flow sequencing. Raised by the user mid-[output-tag-audit].

- The FAQ has no routine update trigger. Surfaced when the plan-marker capture raised an obvious FAQ-entry need and "when do we routinely update FAQ?" had no good answer: new entries happen only when a /plan-authored batch explicitly includes one (observed once, [narrate-build-md-purpose]); the pre-push sweep checks existing FAQ answers against current procedure docs for drift but never asks whether new concepts lack coverage entirely. The gap is the FAQ twin of the spec-entry trigger — [spec-entry-trigger-rethink] keys SPEC updates on "would SPEC.md become wrong or incomplete"; the FAQ needs an equivalent moment, likely at /plan batch-authoring ("would this change raise a question a non-coder would ask? add a FAQ entry to the batch") and/or the push sweep extended from drift-check to coverage-check. Trigger case: queue-visible plan markers, which would put a literal marker line in consumers' queues that the FAQ should explain. The trigger only fixes the future — a one-time audit batch (FAQ coverage against the current plugin: every shipped concept a non-coder would ask about, checked for an answer) is also needed to catch the existing backlog.

- Thinking work still leaks into /next sessions, and its decision-record has no guaranteed home — blocking the autopilot goal. Observed in [output-tag-audit]: the batch seeded a check-whether-already-resolved item (the _build.md ticking [SILENT] concern); resolving it was reconciliation — its output was a decision (drop, already covered), not a finding — and the reasoning landed in _build.md Changes and the eventual audit LOG entry, not where a future planning session would look for the fate of a queued concern. done-audit's LOG template mandates recording captured findings but not dropped ones or their reasons. Design goal to record (user-stated): /next sessions should trend toward pure execution — all thinking work in /plan — because the end state is an autopilot mode that cranks next → done → next unattended until the queue's building is done; the parked cruise-control capture is that mode, and this principle is its prerequisite. Implications for /plan to work through: don't seed audits with reconciliation items (resolve them at planning time); mandate dropped-findings-with-reasons in audit LOG entries; consider a SPEC Principles line for the execution-only /next direction. Same arc as the plan-marker capture filed this session — both mechanize judgment out of /next.

- Audit findings should file automatically as captures — remove the per-finding capture-or-drop loop from /next audits. Raised mid-[output-tag-audit], extending the thinking-work capture above: the present-decide-approve loop is itself mid-/next planning work, and it duplicates /plan — every surviving finding is judged twice (capture-or-drop in /next, then promote/park/drop in /plan). Redesign: the audit reads, compiles, and appends all findings to Captures mechanically; judgment happens once, in /plan, where the user already processes captures one at a time with full discussion. Drops move to /plan too, which fixes the dropped-findings record gap structurally — drop reasoning lands in /plan LOG entries via existing machinery, no audit-template change needed (revises that implication in the capture above). User control is preserved where it binds: findings are deterministic applications of criteria the user approved at batch-authoring, and nothing becomes active work without /plan approval. Makes audits the most autopilot-able session type instead of the least — currently they're the most interactive /next shape there is. Two design details: the filing-time approval exemption must be written into plugin-behaviour.md's Captures rule as an explicit carve-out, not only into the audit procedure — a rule saying always-show against a procedure saying file-automatically is exactly the priority conflict this audit hunts; and the audit close still reports the filed count so the session isn't silent about its own output.

- [output-tag-audit] finding: Execute [SILENT] in next-build.md:5 and next-test.md:5 scopes over sub-flows that require output — next-build.md:22 (user-run test handoff), :28-30 (note failures, continue), :36 (scope-expansion ask "say so first... Wait for approval"), :40 ("say so immediately"); next-test.md:10, :16-18, :24 mirror. Under the hardened [SILENT] ("output zero text... the whole tagged step") this is a direct contradiction: a literal-minded weak model either violates the tag or suppresses a failure report to honour it. Fix shape: re-tag the speaking sub-flows ([PROMPT] for the scope ask and user handoff, [BRIEF] for failure noting) so step-level-overrides-phase-level precedence carries it, plus one local scope line on Execute ("silence governs success-path bookkeeping; failures, asks, and handoffs speak"). The one finding from this audit needing local hardened wording, not just a tag swap.

- [output-tag-audit] finding: plan.md Step 1's interaction moments are untagged. plan.md:36 entry question ("Ask: ...") and :38 follow-up ("Anything else, or ready for Captures?") are stop-and-wait moments with no [PROMPT]; plan.md:34's scan block says "collect them silently" in prose with no [SILENT]. Tag-only fixes — the hardened definitions carry the rest. Interplay: [plan-step1-sequencing] already rewords :36/:38; the tags should ride along in that batch rather than ship separately.

- [output-tag-audit] finding: next.md's halt and gate moments are untagged. next.md:13 push-marker halt ("halt: tell the user...") has no tag — candidate [BRIEF]; next.md:15-21 blocker gate's surfacing moments ("Surface them", "Recommend /plan if found", "The user can confirm, skip, or defer") have none — candidate [BRIEF] for the surfacing, plus [PROMPT] where a user decision gates proceeding (unconfirmed tests; capture-overlap recommendations). Tag-only; the definitions carry the constraints.

- [output-tag-audit] finding: the scope-growth asks lack [PROMPT]. next-build.md:60-65 ("Scope grows during the build" — "ask to add", "propose splitting") and next-test.md:41-46 ("Test surfaces unexpected scope") describe asks that must wait for the user, with no tag. The sibling section "User raises something out of scope" carries [PROMPT] in both docs — the asymmetry is exactly the drift the audit predicts. Tag-only fix.

- [output-tag-audit] finding: the capture-routing approval loops lack [PROMPT]. next-audit.md:17-19 (Route approved findings: draft → approval → append), done-build.md 1.3, done-test.md 1.2, done-audit.md 1.2 (route findings/stragglers to Captures) all run draft-for-approval loops with no tag. Candidate [PROMPT], partial — the routing prose stays. Interplay: if the audit auto-file redesign (capture above) ships, next-audit.md's loop disappears entirely and the done-family loops shrink — apply this after that design lands.

- [output-tag-audit] finding: the /done close-outs leave bookkeeping and ask moments untagged. The unticked-entries ask ("Ask — finish (/next) or close partial") at done-build.md:11-13, done-test.md:9-11, done-audit.md:9-11 needs [PROMPT]. The staleness sweep at done-build.md:55-61, done-test.md:44-50, done-audit.md:44-50 is silent-when-clean, speak-when-flagging — needs the path-split treatment ([SILENT] clean path, [BRIEF] flag path) that next.md:11 already models. done.md:5-13's router ("The check is automatic — don't ask") is [SILENT]-shaped prose with no tag. Minor [SILENT] candidates: registry update (done-build.md:15-20), _build.md deletion (2.3 in all three sub-docs). Tag-only fixes throughout.

- [output-tag-audit] finding: prose restating tag definitions, now redundant post-hardening — with a tension to resolve. plan.md:12 ("One item at a time. Finish one before presenting the next."), plan.md:48 (one-at-a-time/never-preview/count-upfront restatement — the ordering content in that line is step-specific and stays), plan.md:75 ("One at a time"), plan.md:101 ("don't narrate the absence" after [SILENT]), next-audit.md:15 ("Don't preview upcoming findings" after [SEQUENCE]). The tension: trimming prevents drift (the audit's own premise — prose substitutes drift; tags are canonical), but the compliance research argues local restatement reinforces weak models. Proposed line: trim where the hardened definition fully covers the content, keep where the prose adds step-specific substance. setup.md's restatements (:111, :138) are explicitly excluded — see the structural finding below.

- [output-tag-audit] finding (structural): setup.md's tags reference definitions its sessions may never load. The tag definitions live in plugin-behaviour.md, injected only in adopted projects; /setup's Case A/B runs unadopted, so [BRIEF] (setup.md:17), [SEQUENCE, PROMPT] (:109), [BRIEF, PROMPT] (:128) are undefined tokens there, and the prose restatements ("Ask these one per message. Wait for the answer" :111, "One question per message. Do not bundle." :138) are the only carriers — load-bearing, not redundant. Decision needed: keep setup.md deliberately self-contained in prose (consistent with the [trickle-up-ask-when-unsure] precedent that setup.md carries its own copies by design — then the tags are decoration and could even drop), or inline compact tag definitions into setup.md. Either way setup.md is exempt from the restatement-trim finding above.

- [output-tag-audit] finding: setup.md's untagged interaction spots — contingent on the structural decision above. setup.md:14 Case C ("Tell the user, offer to run /plan instead, and stop here") is an offer-and-wait with no tag — candidate [BRIEF, PROMPT]. setup.md:28-107 Step 2 scaffolding is mechanical work narrated later at Step 4.3's file list — candidate [SILENT]. Whether these land as tags or as self-contained prose follows from how the setup.md tags-undefined question resolves.
