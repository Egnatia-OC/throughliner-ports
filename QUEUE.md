...
- Filed at the 2026-06-12 /done close (plan session): discuss whether the LOG entry, the index line, and the commit title/body genuinely need to be three different texts. /done currently authors all of them from the same session facts and presents each for approval, and the user sits through every rendition. Candidate collapse: reuse the LOG entry body as the commit body and the entry's one-line summary (or the index line) as the commit title — one authored text, one approval pass, a shorter /done. Reasons they might still need to differ, to weigh at /plan: the index line is tuned for a later open/skip retrieval decision, not narration; git commit titles have length conventions that index lines ignore (this session's index line is several times a conventional title); and a commit message must stand alone in git history — though the LOG entry file ships inside the same commit, so the body could arguably just point at it. One supporting fact: the hash backfill resolves entries via git log -S on the entry title, so reusing the entry title as the commit title might make that lookup more robust, not less. Decision is /plan's.

- Raised by the user at the [doc-crossrefs-by-name] /done close (2026-06-12), right after filing a capture in the session's post-commit tail. Captures filed after /done's commit have no session record. The LOG entry's "Routed to Captures:" line is written, approved, and committed before they exist — this session's entry says "none," yet two captures originated in its tail (the close-out-assessment capture and this one). They don't naturally belong to the next session's entry either: the log records captures by when they came up, and they came up here. This interacts with the collapse capture directly above (one authored text for LOG entry, index line, and commit message): if that collapse ships, the single text is finalized even earlier relative to the tail, so the gap widens — by the time a post-close capture lands, the record has sailed. The user's tentative landing: the next skill invocation's LOG entry picks up the previous session's post-close captures. Other candidates to weigh: the small follow-up commit that carries a post-close capture also updates the just-written entry's Routed-to-Captures line; or accept the gap, since QUEUE.md itself holds every capture with its origin stated inline. Design is /plan's call, and it should be weighed together with the collapse capture, since both shape the same close-out texts.

- Raised by the user at the [git-add-safety-hook-gap] /done (2026-06-12). Deferred tests have no execution trigger. The section makes pending tests visible — /next re-presents them at every pre-flight — but nothing tasks anyone with producing the confirming event. "Confirmed by:" lines describe observations that only happen if someone deliberately acts or happens to notice; the user is not reading the section closely, consumers won't either, and a weak-model session won't connect a mid-session event to a list it saw at session start. The section was built to replace log-prose deferrals that never resurfaced ([deferred-tests-structural-home]) — surfacing is solved, executing is not. Trigger flavors differ (user's observation): reinstall-gated tests, the dominant flavor in self-hosting, have a mechanically detectable runnable moment — session_start already reports a version mismatch in any adopted project when the installed plugin differs from .si-version, so the first session after a plugin update could present those tests as now-runnable and offer to run what Claude can. User-run checks and external-event tests have no detectable moment; for those, re-presentation may remain the only mechanism, or they need their own answer. Context for any redesign, currently written nowhere: tests are authored in their batch and routed here at /done, so the section only ever holds verification for work that has shipped — failed runs and emergent test needs deliberately route to Captures instead, keeping /plan's ownership of new work. The mechanism is deliberately test-only: unfinished or blocked build work returns to the queue (partial close, Blocked by: headers, push markers), so only shipped-but-unverified tests lack a home elsewhere. Design is /plan's call.

- Observed during the [git-add-safety-hook-gap] build (2026-06-12). The scope-lock parser already strips trailing annotations from Files: lines when a dash separator is used (" — ", " - ", " – "). Parenthetical annotations are not stripped — that is why the observed failure case failed. So the code is partially tolerant while the decided rule says bare paths, nothing else on the line. The two now disagree: a dash-annotated line works silently, a parenthetical one breaks scope. The denial message and next.md now teach the bare-path rule, which the dash-stripping quietly contradicts. Question for /plan: remove the dash-stripping so the bare-path rule is the single truth (a tightening, out of scope for the build that noticed it), or keep it as accepted, undocumented leniency. The build changed neither — it only made the denial teach.
