...
- Deferred/unconfirmed tests have no structural home. /next pre-flight asks "unconfirmed tests from a previous build?" but defines no place to read the answer from — this session the check ran on conversation memory seeded by one prose line in a LOG entry, and both currently-deferred host-side tests (done-split single-summary, _build.md narration moments) are now archived in log-v1.10.0.md, where a fresh session's pre-flight has no instruction to look. Observed during the 2026-06-10 long session: memory covered the gap four pre-flights running; on short sessions with the weaker model the tests would silently never surface. Fix shape: a structural slot the gate reads mechanically — e.g. a "Deferred tests" list in QUEUE.md that /done writes when a test can't run (host-side, needs-user, etc.) and /next's gate reads and re-presents; entries removed when confirmed. Both pending tests should seed the list when it lands.

- Push ritual archives [HASH] placeholders unless session memory intervenes. The push-and-rezip caps LOG/log.md into log-v<VERSION>.md (step 4), but the most recent /done entry still carries its [HASH] placeholder at that moment — the hash only exists after that session's commit, and the backfill belongs to the *next* session, which never sees the archived file. This is how log-v1.8.0 and log-v1.9.0 shipped stale placeholders that a later session cleaned by hand. The 2026-06-10 v1.10.0 push avoided it only because Claude remembered the prior cleanups and backfilled before capping — a move the ritual doesn't contain. Fix shape: add a backfill step to CLAUDE.md's push ritual (host-side, manual-update path) before the log-cap step. Resolved as a side effect if [log-hash-backfill-in-done] or [hash-backfill-as-hook] lands first — both eliminate placeholders before push time; whichever moves first should absorb or close this.

- Long capable-model sessions mask short-session gaps; the design target needs stating and exercising. Observed 2026-06-10 (first Fable day): the balance tipped from new-session-per-skill to one long session, and three procedure gaps surfaced only because session memory had to cover them (deferred-tests home, push-ritual backfill, the check-captures-for-prerequisites trigger when the push marker comes up next). The risk isn't designing *for* the capable model — it's that gaps stop hurting under it, so they stop getting found and filed, while the plugin's consumers and the post-June-20 development model (Opus 4.6, short sessions) still hit them. Fix shapes, combinable: (a) state the design target explicitly in this project's CLAUDE.md Rules — the system must work for fresh short sessions on the weaker model; conversation memory is a convenience, never a dependency; (b) sharpen the existing route-observations rule to name this trigger — any moment Claude notices memory covering for something the docs/files should carry is a mandatory capture; (c) a regression discipline — periodically (e.g. before each push) run a full plan→next→done cycle in deliberately fresh sessions, or on the fallback model, to keep the session-boundary contract exercised.
