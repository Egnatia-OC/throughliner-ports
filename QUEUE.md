...
- **[install-note-to-claude-visible-to-user]** INSTALL.md's "Note to Claude" frontmatter and ground rules block sits at the top of the file. Every persona read it as the first thing in the guide and bounced off it — CS thought the chatbot was about to do something on their computer, DAC wondered if it was for them, FP read it as visible stage directions, AI registered "wait, what am I looking at." It's AI-facing prose in the position the human reads first. Same problem applies to the pacing rules block at the bottom (CS: "pacing rules for who?"; AI: "reads weird as a guide"). Fix shape: either move AI-facing instructions into a separate file the human never sees and link from INSTALL.md, or open INSTALL.md with a one-line "this guide is meant to be pasted into a Claude chat; the first block is for Claude, you can skip it" — preferring the former. Surfaced by CS + DAC + FP + AI.

- **[install-claude-code-vs-chat-app-disambiguation]** The Claude Code / Claude chat app distinction is load-bearing for routing but stated only in one sentence inside Step 1 Q2 ("Claude Code is a separate app from the Claude chat app at claude.ai"). DAC read straight past it — "I have the Claude app, that's the desktop app, moving on" — and confidently misrouted to Branch B, then crashed at the Customise menu hunt without ever realising they were in the wrong application. CS couldn't tell which app was which from the "check Applications folder" hint because both Claude apps live there. Fatal failure mode for DAC: the install guide gives no recovery path; the helping assistant would dutifully troubleshoot a missing Customise menu rather than diagnose wrong-app. Fix shape: lead the guide (not bury in Q2) with a screenshot or icon comparison of the two apps, or add a Q2 sub-check that forces a positive identification ("open the app and tell me the exact name in the title bar"). Surfaced by CS + DAC (catastrophic for DAC).
