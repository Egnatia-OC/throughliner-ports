...
- Build scope has no canonical definition; add one anchor statement to plugin-behaviour.md. From [scope-distinction-audit]. The working definition is currently assembled from three places: next.md "The entries are the contract. Don't exceed the described work" (Rules section), next.md Step 2 (the Files: section feeds the scope-lock; the hook allows listed files plus method docs), and next-build.md "Stay within the entries' described work". plugin-behaviour.md says "Don't fix things outside current scope" but never defines current scope — and it is the only doc injected into every session, so a session can hold the rule without the definition. The gap sharpens when [trickle-up-next-md-duplicates] ships: that batch deletes next-build.md's restatement, leaving the undefined term as the rule's only statement. Proposal: a short Scope statement in plugin-behaviour.md as the single anchor — scope is the active batch's entries' described work; the Files: list is the mechanical proxy the pre_tool_use hook enforces; work outside it routes to Captures. Skill docs point at the anchor instead of restating it. Locate all passages by content, not line numbers — the docs shift under other queued batches.

- Scope is enforced at two layers that no doc connects. From [scope-distinction-audit]. Work-scope is "the entries' described work" (next.md Rules; next-build.md Rules during build), enforced by judgment. File-scope is the Files: list in _build.md (next.md Step 2), enforced mechanically by the pre_tool_use hook. Nothing states the relationship: the file list is a proxy, and the hook a backstop, not the definition. The gap is real — a build can stay within listed files yet exceed the described work inside one of them, and the hook stays silent. The divergence already shows in the docs: the hook allows REGISTRY.md and QUEUE.md edits as method docs while next-build.md says "REGISTRY.md is not build scope"; and the scope-expansion ask template is file-shaped ("I need to also edit [file]") while the rule it serves is work-shaped — no ask covers work growth inside an already-listed file. Proposal: one or two sentences at the scope anchor (sibling capture from this audit) — name both layers, state that the entries' described work is the definition and the file list its mechanical approximation, and that passing the hook never by itself makes work in-scope. Locate by content, not line numbers.

- Claude-noticed out-of-scope discoveries have no defined routing move. From [scope-distinction-audit]. The docs route discoveries three ways depending on who noticed. User-raised items get the full capture flow — draft, show for approval, "anything else?", resume (next-build.md and next-test.md Scope management). Isolated test failures get "route the fix to Captures at close" — destination and timing stated (both Execute sections). Claude-noticed unrelated problems get only "Note them for the queue" (next-build.md Rules during build, next-test.md Rules during test, plugin-behaviour.md Routing and discipline) — no destination section, no approval mechanics, no timing. "Note" could mean append to Captures now, hold for close, or hold in conversation memory — and the last is the exact failure the short-session design target forbids. "Nothing unrouted survives a session" only backstops items that survive to close. Proposal: replace the verb in all three spots with one defined move with stated timing — e.g. route to Captures at close per the capture flow — matching the clarity the other two paths already have. Locate by content, not line numbers.

- The decision rule for mid-build discoveries is real but never stated. From [scope-distinction-audit]. next-build.md resolves discoveries along one axis: needed to complete the batch's entries → ask to add (minor) or propose splitting (significant); not needed → capture and continue; premise broken → stop and course-correct. The axis is only visible across five separate passages (Rules during build, Scope management's two sub-sections, the user-test discovery path, Course-correction) — no doc states it as one rule. The classification step is where judgment sits: "unrelated," "minor," "significant" each carry a call, and nothing tells a weaker model which question to ask first. This is the audit batch's named failure mode from both directions — halting for something that should have been queued is mis-classifying not-needed as needed; absorbing something silently is the reverse. Proposal: one-line decision rule at the head of Scope management in next-build.md and next-test.md, or at the scope anchor pointed to from both — needed → ask or split; not needed → capture and continue; premise broken → halt. The existing sections then read as elaborations of one rule instead of five sibling cases. Locate by content, not line numbers.

- The word scope is polysemous across the rulebook, unmarked. From [scope-distinction-audit]. In the /next family scope means build scope — what the active session may touch. In plugin-behaviour.md Dependency ownership, "The user owns scope — what enters the queue, what gets parked/dropped, whether to proceed" means project scope — what work exists at all. The senses are compatible (the user owns what work exists and whether a build expands; Claude enforces the lock and asks) but nothing marks the switch, so the reader disambiguates by context alone. A weaker model can blur them in either direction: treating every scope question as the user's to answer and asking when it should capture-and-continue, or reading the user's in-conversation wishes as overriding the lock without the formal ask. Proposal: disambiguate at the scope anchor with one line naming both senses ("the user owns what work exists; build scope is what the active batch's entries describe"), or reword the Dependency ownership bullet to avoid the word ("the user owns what enters the queue, what gets parked or dropped, and whether a build expands"). The reword is the smaller change; the anchor line does more teaching. Locate by content, not line numbers.

- The authoring side of scope is unstated in plan.md. From [scope-distinction-audit]. Batch entries written in /plan become /next's enforcement contract: the "described work" rule binds to their wording, and the scope-lock's Files: list is populated from "the files the batch entries name" (next.md Step 2). plan.md Step 3 says only "Each entry names its own target" — it never states that entries feed the lock, and never requires build entries to name the files they touch. File-naming is currently convention from this project's habit, not rule. A consumer's /plan or a weaker model can author a sensible-reading build entry with no file paths, and the lock then has nothing to hold — the Files: section comes up empty for a batch that does edit files, and scope gets decided by guesswork at lock time instead of at planning time. Proposal: one sentence in plan.md Step 3 near the batch structure — build entries name the files they touch; the scope-lock is populated from them; an entry naming no files leaves the lock empty. Optionally fold the same check into the batch readiness gate, which already asks whether the candidate index entry can be written. Locate by content, not line numbers.
