...
- setup.md Step 4 close-out (line 134) assumes /next is always build-shaped and always available: "Tell them: 'Run /plan to scope your first batch, or /next if you're ready to build.'" Two fixes needed. (1) Wording: apply the same inclusive treatment as Q4 — "…or /next if you're ready to start the first batch." Type-neutral, no "build" assumption. (2) Conditionality: only offer /next if Q4 actually produced a usable first batch. Q4 currently creates a Build entry, but if it was skipped or unusably vague there's no batch to run — the close-out shouldn't blindly offer /next in that case. From [behaviour-agnosticism-audit].

- setup.md Q4 assumes the first batch is a build batch with runtime behaviour: "What's the first thing to build? What would you want working by the end of today?" For a tax-prep project the first thing might be "sort the year's receipts into folders" — not a build, nothing to "work." Connects to parked [user-execution-batch-shape] and queued [ship-freeform-next-type] — the right framing partly depends on whether non-build batch types exist at /setup time. But the wording can be loosened to be inclusive (not replaced — dropping "build" would alienate non-coders building with a codebase). Proposal: "What's the first thing to build or do? What would you want to have working or made progress on by the end of today?" Keeps the build-shape framing for app projects, adds the do/progress framing for non-app projects. From [behaviour-agnosticism-audit].

- setup.md Q3 examples are all software-product constraints: "Any principles or constraints? (e.g., 'must work offline', 'no accounts', 'mobile-first')." A tax-prep user reading these will either skip the question ("none of those apply") or strain to translate. A records-keeping user might genuinely have "must be readable in 10 years" or "no cloud storage" — different shape, same purpose. Reword example set to span project types — pick 3–4 covering software + non-software shapes so the user sees the question applies regardless of project type. From [behaviour-agnosticism-audit].

- setup.md Q2 assumes the project is software with runtime functionality: "Describe the core functionality — what does it actually do?" A records-keeping project has structure and purpose, not functionality. A research project produces findings, doesn't "do" anything. The framing only works cleanly for software. Reword project-agnostic: "Describe how it works — what's the shape of it day to day?" Or "What's the core of it — the main thing it produces, organises, or does?" The "produces/organises/does" triplet covers software and non-software shapes. From [behaviour-agnosticism-audit].

- setup.md Q1 assumes the user is building something: "What are you building and who is it for?" For records-keeping, tax-prep, research, or writing projects, "building" is the wrong verb — those projects organise, gather, investigate, or draft. The framing pushes the user to either reframe their answer into building-language or feel SI isn't for their kind of project. Reword project-agnostic: "What is this project, and who is it for?" Or two-part: "What is this project for? Who's it for?" Loses the active-verb cue but covers every project type. From [behaviour-agnosticism-audit].

- setup.md REGISTRY.md template (lines 78–82, scaffolded into user's REGISTRY.md) assumes projects have "components" (software-architecture term) and that builds are the only thing that adds to the registry: "Components that exist in this project. Updated after each build." A tax-prep project might register a receipts folder, a lender list, a year-end packet — not components, and entered via audit or freeform work, not builds. Proposal: add a Q3.5-style interview prompt — "what are this project's parts called?" — and use the user's own noun in the scaffolded REGISTRY.md. Decouple the update trigger from "build" too ("Updated as the project grows" or similar). From [behaviour-agnosticism-audit].

- setup.md QUEUE.md template (line 63, scaffolded into user's QUEUE.md) assumes only Build and Test batch types exist: "Each batch is one /next session — builds first, then tests." Audit batches already exist (this audit is one) and freeform is queued. The scaffolded line teaches a wrong shape from day one and ages worse as more types ship. Reword type-complete: "Each batch is one /next session. Subheadings name the kind of work (Build, Test, Audit)." Or shorter: "Each batch is one /next session of work." From [behaviour-agnosticism-audit].

- setup.md SPEC.md template (line 48, scaffolded into user's SPEC.md) assumes every project is an app: "SPEC.md — product truth. What the app is, who it's for, how it works." This line ships into the user's own SPEC.md and is read on every visit — the framing leaks into how they describe their project to themselves. Reword project-agnostic: "What the project is, who it's for, how it works." "Project" is the neutral term setup.md uses elsewhere anyway. From [behaviour-agnosticism-audit].

- setup.md Step 1 folder-state cases assume the only pre-existing content is source code: "Case A — Empty/near-empty: No source code, no docs." / "Case B — Existing code, no method docs: Source files exist…" A tax-prep folder has receipts/PDFs, a records-keeping folder has spreadsheets, a research folder has notes — none are "source code" but all count as pre-existing content for the Case A/B split. Reword project-agnostic — "No content" / "Content exists" or "No pre-existing material" / "Pre-existing material exists." From [behaviour-agnosticism-audit].

- plugin-behaviour.md line 83 spec-entry pipeline assumes the project ships features to an external user: "New features need a spec entry before a build entry. … Threshold: if a user would see or experience the difference, update SPEC.md first." For a personal records-keeping or tax-prep project there are no "features" and the owner is the only "user." Reword project-agnostic — replace "new features" with something like "new scope items" or "new project properties," and broaden the threshold ("if the project's output or shape changes in a way someone would notice"). From [behaviour-agnosticism-audit].

- plugin-behaviour.md line 81 doc routing assumes the project is a product made of components: "SPEC.md = what/who/how/why the product exists. … REGISTRY.md = what components exist." A records-keeping or tax-prep project has neither — it has organised material, not a product with parts. Reword project-agnostic, e.g. "what/who/how/why the project exists" and a neutral noun for the project's constituent things (parts? entries? items?), or demote per-project-type. From [behaviour-agnosticism-audit].
