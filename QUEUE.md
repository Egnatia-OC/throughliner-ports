...
- Observed at the 2026-06-12 /done close (the f05e336 commit step): the commit title and body were presented in fenced code blocks — black, non-wrapping — instead of the blockquote format. The same close had already used a blockquote for the LOG entry draft. The fence happened because done.md's commit core explicitly says "each in its own fenced code block" and points at plugin-behaviour.md's verbatim-copy rule. An explicit local instruction beats a decided-but-unshipped standard at the moment the doc names fences. [approval-display-blockquotes] already covers the fix and lists commit message among its blockquote content types — this observation pins the exact spot its sweep must hit (done.md commit core, step 2) and confirms the verbatim-copy bullet in plugin-behaviour.md needs the rewrite too, not just the approval-time bullet. One supporting fact for that batch: Claude runs git commit itself, so the copy-paste affordance fences exist for doesn't apply to commit messages — readability wins with nothing lost.

- Observed at the 2026-06-12 /next pre-flight — the first one after the verbatim-first decision. The top batch's text did not arrive first. The hash backfill ran first, as tool calls the user sat through with nothing to read. The batch then arrived bundled into one message with the gate findings and the Ready ask. Nothing blocked an immediate send: QUEUE.md was already in context from this session's /plan. This is the exact failure [capture-verbatim-first] documents from its /plan live trial — ordering text on the page isn't enough; the quote must be sent as its own beat before the thinking runs. Now observed on the /next side too, before that batch ships. Same mechanism as the commit-fence capture filed minutes earlier: the installed doc's ordering wins over a decided-but-unshipped standard. One sequencing note: the backfill-first wait dissolves on its own — [hash-backfill-as-hook], in build now, deletes the backfill from /next pre-flight entirely and ships ahead of [capture-verbatim-first].

- Observed mid-build at [hash-backfill-as-hook], 2026-06-12: when plugin-behaviour.md was added to the build's scope with approval, Claude wrote the new Files: line with a trailing note in parentheses ("added mid-build, approved..."). The scope-lock hook matches Files: lines as exact paths, so the annotated line silently failed to grant scope and the next edit was denied. The deny message listed the allowed files, which made the cause visible — but only after the failed attempt, and a weaker model might loop on retries instead of spotting the formatting cause. Nothing documents that Files: lines must be bare paths, one per line, nothing else on the line. Two possible fixes, not mutually exclusive: state the bare-path rule in next.md's working-file template (cheap, ships with the docs), and/or make the hook tolerant of trailing annotations when matching (riskier — looser matching on a safety mechanism). The deny message could also name the rule ("Files: lines must be bare paths") so the recovery is self-documenting at the moment of confusion, same move [git-add-safety-hook-gap] makes for the git-safety denials.

- Observed at [hash-backfill-as-hook], 2026-06-12: deleting next.md's pre-flight backfill sub-step renumbered the remaining steps, and step-number cross-references in other docs broke silently — three /done sub-docs said "(mirrors next.md Step 1.4)" and plugin-behaviour.md said "at /next Step 1.4 blocker gate", all pointing at the blocker gate that had become Step 1.3. The build caught and fixed them only because the session chose to grep for step references before renumbering; no procedure requires that check, so a weaker model's renumber would leave dangling pointers in docs every session loads. This recurs by construction: procedure docs cross-reference each other by step number in several places, and any batch that adds, deletes, or reorders steps re-creates the break. Queue coverage today: none — [queue-format-lint-hook] lints QUEUE.md only, and the push ritual's consistency sweep checks templates and project docs, not doc-to-doc step references. Candidate fixes for /plan to weigh: (a) an authoring rule that procedure docs cross-reference by name ("the blocker gate in next.md's pre-flight"), never by step number — names survive renumbering — plus a one-time sweep converting existing numeric references; (b) a process rule that any renumbering edit greps the docs for step references first; (c) lint-hook detection of dangling step references. Same fragility class as the "locate by content, not line numbers" rule the queue already applies to batch entries — this is its procedure-doc twin.

- Observed at [queue-format-lint-hook] build testing, 2026-06-12: the prose-citation check re-fires on every QUEUE.md edit for references it has already flagged. The check is stateless — it reads the whole file, not the edit — so a reference the user has judged "citation, fine" gets flagged again on every later edit, indefinitely. The build already cut the worst noise by flagging only references to slugs still defined in the file: a dry run showed references to shipped work were the bulk (~2.5KB of advisory text per edit), and a reference to shipped work can only be a citation. The residue is real: the first live run flagged ~21 batches, all naming pending items, and any judged-as-citation among them will keep re-firing. Candidate fixes if live use shows this as noise: an inline convention marking a reference as a deliberate citation, which the lint would skip (new format design, /plan's call); or accepting the re-fire as cheap advisory background. Decision deferred until the hook has run live in a few /plan sessions.
