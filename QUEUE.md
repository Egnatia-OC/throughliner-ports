...
- Criteria: any step whose prose describes verbosity or interaction shape (e.g. "say nothing," "briefly note," "ask the user," "discuss tradeoffs," "one at a time") where the matching tag would carry the same intent more cleanly. Also flag tag misuse — a tag applied where the step's prose contradicts it, or a tag missing where the step's behaviour clearly needs one. For each finding: quote the prose, name the candidate tag, note whether replacement is full (tag alone) or partial (tag + retained prose). Account for the priority architecture (resources/research/model-instruction-compliance.md): skill docs are user-message priority, so a bare tag can lose to system-prompt helpfulness training — for each finding, also note whether the spot needs compliance-hardened wording (a why-clause or positive quantified constraint) for the tag to hold on weaker models, not just the tag swap. Write findings assuming the tag definitions themselves may be rewritten per [tag-definitions-compliance-rewrite].
