...
**After build.** `MANIFEST.md` is updated for anything created/renamed/deleted. A plain-English changelog is provided ("I am adding a check to the age field so people can't enter negative numbers"). The user is prompted to test and switch back to planning mode.
...
The method ships with the author's preferences and commitments embedded in `NO-CODE-METHOD.md`. A new user adopting the method needs to distinguish three layers:

**Method contract — load-bearing, edit at your peril.** Some lines read like personal preferences but the method's machinery depends on them. "I'd rather be told I'm wrong than agreed with" — the drift checks, red-flag surfacing, and planning recaps all assume Claude will push back on the user. "Don't stealth-fix regressions" — the after-build changelog assumes Claude states regressions plainly. "Don't immediately fold under push-back" — planning recaps assume Claude will defend its reasoning before changing it. Adapt the phrasing to your voice if you want; do not delete the substance.

**Personal workflow — edit freely.** Some lines are genuinely the author's habits between Claude Code sessions: `/clear` after each build, prepare test results as pasteable text, review all upcoming changes before each build. A different user with a different review rhythm should rewrite these to match how they actually work.

**The build sequence — fixed, not advisable to personalise.** The four-phase build cycle (At session start → During planning → Before build → After every build) is the method's spine. It is not the same thing as personal workflow commitments, and it is not part of the editable surface. Don't confuse the two.

A future version will fence these three layers explicitly in the spec; until then the lines sit unfenced in `NO-CODE-METHOD.md`.
...
**Push-back without folding.** The rule "if I push back on a suggestion, do not immediately fold and do not dig in either" is load-bearing. Capitulating without engagement is no more useful than refusing to listen — both bypass the conversation that decides whether the original suggestion was right. (See *What's editable, what's not* for why this is method contract, not personal preference.)
