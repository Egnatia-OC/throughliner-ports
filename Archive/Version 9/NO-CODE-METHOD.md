...
- **No placeholders.** Decided behaviour only. Open questions live in `BACKLOG.md` as planning batches, not as inline `[TO FILL IN]` or `[Open: ...]` notes inside the doc itself. This is the same rule `UX.md` follows, extended to cover all source-of-truth docs.
...
- **[SILENT]** Read CLAUDE.md, UX.md, BACKLOG.md, and MANIFEST.md before responding to anything else. If the project has any additional source-of-truth docs, read those too. Those docs hold the project state.
