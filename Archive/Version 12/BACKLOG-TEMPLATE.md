...
Serves UX.md: [entry name(s)].
[For batches touching an additional source-of-truth doc, add a `Serves <DOC>: ...` line. Use `Serves <DOC>: [entry/section name].` when the batch implements the doc's content, or `Serves <DOC>: [delivery mechanism].` when the batch's purpose is to carry the doc to its runtime destination rather than implement 