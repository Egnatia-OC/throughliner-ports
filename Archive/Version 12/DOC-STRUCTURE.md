...
- **Locked during builds.** Same rule as `UX.md` — see `NO-CODE-METHOD.md` → *When to read each document*.
- **No placeholders, no soft gestures.** Source-of-truth docs describe decided behaviour. Don't write placeholders (`[TO FILL IN]`, `[Open: ...]`) and don't write sentences that gesture at the doc's own undecidedness ("currently undecided", "pending decision", "to be revisited", "see `BACKLOG.md` for the open question"). The status of an open question lives in `BACKLOG.md` only, not inside the doc body. If a default behaviour applies while the question is being resolved, state the default plainly without flagging it as provisional — runtime audiences need the operational instruction, not its implementation status.
...
Articulating the why before the how protects against feature drift and makes scope decisions easier later.
...
*Example entry using* **Risk accepted***:*

> **One-day-at-a-time view**
> The Today screen shows only the tasks scheduled for the current day. There is no week view, no agenda, no "next 7 days" peek. To see what's coming, the user moves through days one at a time using a forward-only arrow.
> The user needs this because looking ahead at a wall of upcoming tasks is the planning-pressure that triggers shutdown for users with executive dysfunction. Seeing only today's load keeps the cognitive surface area small and the bar for getting started low.
> **Risk accepted:** users cannot plan around upcoming busy periods or anticipate scheduling conflicts. We've judged this an acceptable cost — the people this app is for are already overwhelmed by ahead-planning; protecting the present is more important than enabling the future.
