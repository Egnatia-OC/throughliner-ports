...
This file holds the structural specifications for the project's documents. It is reference material consulted during planning, migration, or when writing or editing doc entries — not on every build.

For the high-level role each doc plays and the rule about when to read each one, see `NO-CODE-METHOD.md` → *The documents that describe my projects*.
