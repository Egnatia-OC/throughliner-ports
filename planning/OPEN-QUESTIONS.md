...
## V27 smoke testing owed — Windows integration tests not yet run

**The question.** V27 shipped substantial new code — `after-build` subagent (NEW), `pre_tool_use.py` check (f) test-confirmation gate (NEW), `session_start.py` TEST-LOG tripwire (NEW), `stop.py` after-build routing (NEW), `batch-executor.md` scope-reduction, `planning.md` per-row read-back + inline `[Requested]`/`[Suggested]` label-writing, `before-build.md` label preservation. None of it has been smoke-tested in Claude Code. The V27 BUILD-LOG entry's *Carried forward* section names the ~13 owed checks as deferred to Alex's post-commit Windows run, but at V27 close the run was explicitly deferred to a fresh session. Should V28's scoped work proceed first, or do V27's smoke tests run first?

**Why it matters.** Surfaced at V27 close (2026-05-17). Untested code in load-bearing surfaces is structurally risky: the test-confirmation gate (check f) gates whether new builds can start; the SessionStart tripwire overrides main Claude's normal routing. Building V28 on top of unverified V27 would mean any V27 bug surfaces later, mixed up with V28 changes, much harder to diagnose. The V25 pattern (per its BUILD-LOG and the *Don't propose "live install + back-test" as a session* memory entry) is post-commit smoke-tests via `claude --plugin-dir <scratch>`, outcomes appended to `TEST-LOG.md` as a follow-up commit — that pattern hasn't been completed for V27 yet.

**Working notes.**

- The ~13 owed checks: 5 around the gate (denies on unconfirmed previous-session rows, denies on missing BUILD-LOG → strict fallback, denies on unparseable BUILD-LOG → strict + parse-failure-named reason, permits on clean TEST-LOG, SessionStart tripwire fires only when pending rows exist); 3 around after-build (silent MANIFEST update, blank-Status TEST-LOG row writes, recap with `[Requested]`/`[Suggested]` labels read off BACKLOG.md); 2 around planning extension (per-row read-back as first sub-step, push-back on bulk confirmations); 2 around labels (planning writes inline into BACKLOG.md change-list, before-build preserves across halt-C splits); 1 around the DOC-STRUCTURE.md edit (visible in spec at *Build batches → Change list — `[Requested]`/`[Suggested]` labels*).
- Pre-validation in this Cowork session was limited to a PowerShell `python -m py_compile` round on the three hook scripts (all clean). The bash-mount stale-view issue blocked deeper bash-side checks (see `reference_cowork_git_mount_phantoms.md` — read-side failure mode reproduced V27).
- V28.md as currently written has separate staleness — it asserts the plugin "has never run inside Claude Code as of V23" and treats live testing as deferred-future-work. Both contradicted by V25's `claude --plugin-dir` smoke tests of `/before-build` and `/build`. V28.md needs a pre-flight scope review when V28 actually starts, independent of the V27-test question.

**Next step.** **Run the V27 smoke tests as the first work in the next Cowork session, before V28 scope begins.** Walk one test scenario at a time per `CLAUDE.md` → *My command prompt experience level*. Each scenario: set up the scratch-directory state (fixture `CLAUDE.md`, `BACKLOG.md`, `TEST-LOG.md`, optionally `BUILD-LOG.md`), run the command, observe outcome, write a TEST-LOG row in `sovereign-implementer/TEST-LOG.md`. Append outcomes as a follow-up commit (no version bump — TEST-LOG appends are dev-internal per BUILD-METHOD.md). Only after the smoke-test pass closes (or each row has a confirmed outcome) should V28 planning begin. **Promote sooner** is not applicable — this entry IS the promotion to "first action next session."
