...
- `plugin/agents/adopt.md` — `/setup` scaffolding logic; gains `research/` folder
...
- Smoke-testable via `--plugin-dir`: run `/setup` on a scratch folder; verify `research/` exists; trigger a research-worthy question during planning; verify the agent writes to `research/`
