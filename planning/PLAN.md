...
| V38+ | Remaining promotion-ready open questions | UX.md non-GUI adaptation; TEST-LOG row pruning (unpaired from ordering after V36); SoT doc edits with no-coder permission (paired with footer-stamp-on-locked-docs from V35); "planning" vocabulary collision with Claude Code "plan mode" (may pair with plan-panel design half if V36 research finds the panel writable); MANIFEST.md schema gap. Scheduled post-V35 so E2E evidence informs shape. Each becomes its own session row when promoted. |

20 sessions through V37, plus V38+ TBD. Some will combine or split during execution; the count is a target, not a contract.
