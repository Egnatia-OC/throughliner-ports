...
| `DOC-STRUCTURE.md` | Plugin + repo root | Bundled reference doc at `plugin/docs/DOC-STRUCTURE.md`; mirrored at repo root (`DOC-STRUCTURE.md`) as part of the docs-only canonical set (V32). Read by planning, before-build, and adopt subagents when their *Mode* tag applies (planning, migration). Two copies — plugin-side cross-refs point at plugin homes (universal-behaviour.md, etc.); docs-only side points at sibling NO-CODE-METHOD.md sections. |
| `VOCABULARY.md` | Plugin + repo root | New in V32. Plugin copy at `plugin/docs/VOCABULARY.md`; docs-only copy at repo root (`VOCABULARY.md`). Holds method-term definitions (planning batch, build batch, test session, etc.) that used to live in `NO-CODE-METHOD.md` → *Vocabulary*. Referenced from subagent bodies, hook deny messages, and other docs. |
| `NO-CODE-METHOD.md` | Repo root (docs-only) | **Retired from plugin in V32.** Lives at repo root as the canonical docs-only / project-agnostic spec — the prose-only version of the method, for use without the plugin. Subagents no longer read it at runtime; operating procedures are inlined into subagent bodies + `universal-behaviour.md`. Two-write rule keeps the docs-only spec aligned with plugin substance (see `BUILD-METHOD.md` → *Two-write rule for canonical docs*). |
