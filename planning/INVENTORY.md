...
- **Skill-with-flags pattern** (older, V19). Skills with `disable-model-invocation: true`, `user-invocable: true`, `agent: <subagent>`. Claude Code v2.1.101 merged commands into skills; this pattern is the result. Used by `/init-project`.

**As of V25, three commands shipped:** `/init-project` (V19, skill-with-flags), `/before-build` (V25, commands-directory), `/build` (V25, commands-directory). Remaining commands below are forward-looking architectural reference with planned-V annotations — a reader (human or Claude) should not assume *Pending* commands exist in the installed plugin.

- `/init-project` → scaffolds 5 spine templates (CLAUDE, BACKLOG, MANIFEST, UX, TEST-LOG) into a user project (ADDITIONAL-DOC handled by `/add-sot-doc`). **Shipped V19**; extended V26 to also scaffold TEST-LOG.md.
...
- 6 templates (CLAUDE, BACKLOG, MANIFEST, UX, TEST-LOG, ADDITIONAL-DOC) under `plugin/templates/`. The 5 spine templates are scaffolded by `/init-project`; ADDITIONAL-DOC lands via `/add-sot-doc`.
