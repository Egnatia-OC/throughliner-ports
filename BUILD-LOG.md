...
**Decisions taken and why.** Non-blocking placeholder wording (`Scope not yet defined — fill during the next planning session.`) chosen over no-scope-sections-at-all because the V47 structure is still useful even for migrated batches — the before-build subagent can read the sections and knows what's missing vs what's present. Square brackets were the specific problem: they pattern-match as template placeholders. The BUILD-LOG migration extracts entries chronologically (oldest first) so per-build file numbers match creation order.

**Pivots and surprises.** None. Both changes were well-bounded by the E2E findings.

**Carried forward.** Out-of-scope staleness: Crash course's case 4 description says "locked docs get `[PROPOSED EDIT PENDING]` entries for the no-coder to bump by hand" — this predates the V38 footer-stamp carve-out (locked docs now get footer bumps directly). Not fixed in this session; flagged for a future doc sweep. All prior deferred smoke tests carry forward to 0068 (E2E round 2).
...
**Decisions taken and why.** Structured fields in per-build files chosen over a separate `performance-log/` folder or conversation-only capture. The per-build file already exists, the after-build subagent already writes it, and collocating performance data with the build narrative keeps related information together. A separate folder would create a parallel log with redundant session identification. Conversation-only capture loses cross-session queryability — the stated goal. Mechanical measures only (no vibes-as-scores) per the OPEN-QUESTIONS entry's V19 honest assessment: "without a mechanical success criterion, 'well' becomes vibes-encoded-as-data." Session notes are optional and user-written to keep the subjective half honest.

**Pivots and surprises.** CLAUDE.md was stale — said v57/V52 but we were at v59/V54 (two sessions behind). Updated at close. No implementation surprises — the scope was well-bounded: one new section in a file the subagent already writes, plus doc parity.

**Carried forward.** Deferred smoke tests: add V55 Performance section to the list (verify after-build writes the Performance section in a desktop-app burner session). All prior deferred smoke tests unchanged.
