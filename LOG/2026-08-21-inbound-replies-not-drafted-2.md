...
One ripple found by the acceptance grep and filed rather than folded in: done-build.md's step 1.5 still carries the old broad trigger — [done-build-reply-trigger-stale].

**Files touched:** `plugin/throughliner/docs/feedback-and-inbox.md`, `plugin/throughliner/docs/skill-nonspecific-rules.md`.
**Routed to Captures:** [done-build-reply-trigger-stale].
Tick: done, confirmed — grep shows one statement per file, both naming the question trigger.
FAQ: not needed because the reply offer already existed; nothing a consumer does changes.
Rule gate: run — the triage gains one routing outcome as an amendment to the existing mail step, and the always-loaded reply rule is reworded rather than duplicated: the old broad-trigger sentence is evicted and replaced by the narrowed one, so no second rule on the subject exists.
