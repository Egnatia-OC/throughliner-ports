...
Rule gate: run — amendment to the rule gate's own admission question 1 in CLAUDE.md, reworded in place; no freestanding rule, nothing evicted. Admitted on the two same-day instances — which also satisfy the outgoing test on its own terms, so the change is valid under either reading.

Files touched: CLAUDE.md; resources/self-authoring-rules.md
Routed to Captures: none
Done, confirmed: grep "more than once" in CLAUDE.md returns nothing.
