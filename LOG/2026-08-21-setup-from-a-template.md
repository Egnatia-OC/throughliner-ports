...
Two of their findings are recorded here because they are worth keeping whatever happens to templates: the useful way to carry a rule between projects is to **generalise to the failure, not the abstraction** — rewrite the incident so the mechanism survives and the people do not; and **a template must never carry a decision the copying user has not made** (their example: a transcript-capturing hook that is fully portable while the consent that justifies it is not). If templates are ever revisited, those two are where the design starts.

**Queue changes:** [setup-from-a-template] deleted from Unprocessed.
**Work processed:** deleted, with an outbound reply sent on approval.
