...
- [HASH] — /next [e2e-install-guide]: 4 parallel stranger-Claude subagent cold-reads of INSTALL.md (cold-stranger / desktop-app-confused / free-plan / already-installed) synthesized into 11 grouped findings appended to unprocessed Captures, each tagged with which personas surfaced it. Catastrophic failure mode surfaced for desktop-app-confused (silent misroute to Branch B → menu-hunt crash with no recovery). Staleness flag: processed [e2e-install-guide reshape] capture now fulfilled.
