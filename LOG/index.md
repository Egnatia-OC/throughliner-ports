...
- [HASH] — /plan session: promoted [narration-vocabulary] (name background-only terms in plugin-behaviour.md with translate-or-omit rule; "loop / Step 2" leakage reframed from a sequence-naming audit to a broader vocabulary rule) and [audit-definition] (generalize audit-batch definition across plan.md + next.md to lead with output contract — findings to Captures, no direct edits — and drop docs-as-target framing; reader is always Claude). Parked FAQ-for-_build.md behind [reader-test-refresh]. 2 captures: inline-marker mechanism (conditional on whether vocabulary rule alone holds), freeform /next type for handmade-changes wrap-ups.
