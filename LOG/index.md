...
- [HASH] — plan.md Step 1 and next.md Step 1.1: rewrote LOG hash backfill to grep-gate → batch-read → `git log -n 1` common-case → `git log -S` fallback; dropped `--diff-filter=A`/blame
