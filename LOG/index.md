...
- [HASH] — /plan: [e2e-install-guide] reshaped from single user-run live-chat E2E to 4 parallel stranger-Claude subagent sims (cold-stranger, desktop-app confusion, free-plan, already-installed), cold-read not interactive, cross-scenario synthesis to Captures, slug+position preserved. [user-edits-rollup-on-commit] filed: /done's per-build commit only stages build-touched files, leaving user-made target edits dirty across sessions.
