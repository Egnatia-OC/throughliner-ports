...
- [HASH] — /next pre-flight on [done-closeout-extraction] fired the blocker-gate capture scan on [trickle-up-done-md-file-safety]; /plan folded it in — rationale sentence + constraint Build entry (no file-safety restatements in done.md's commit core or the four sub-docs; procedural steps stay) — and removed the capture from Captures. Hashes backfilled to 3526cde.
