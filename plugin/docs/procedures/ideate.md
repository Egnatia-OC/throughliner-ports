...
2. `BACKLOG.md` — scan for existing batches, OQs, and Ideas section. Needed to avoid duplication and find natural homes for new ideas.
...
4. **[SILENT] Route the idea.** Based on discussion:
   - **New OQ** — write full OQ entry in BACKLOG Open Questions section. Include `Surfaced` tag with current session.
