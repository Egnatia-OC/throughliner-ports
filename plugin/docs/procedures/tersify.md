...
**Planning phase only.** Source-of-truth docs must be unlocked. If a `Status: active` batch exists in BUILD-PLAN, deny with: "Source-of-truth docs are locked during build. Run `/sovtersify` in a planning session."

## Phase gate

Before anything else, check BUILD-PLAN for an active batch:

1. Read BUILD-PLAN (resolve path from `CLAUDE.md` path block — `_method/proxies/build-plan.md`, `BUILD-PLAN/INDEX.md`, or `BUILD-PLAN.md`).
...
**Step 1 — Collect the doc list.** Read `CLAUDE.md` path block. Collect all source-of-truth doc paths: `UX.md`, `MANIFEST.md`, additional docs declared in the path block, `CLAUDE.md` itself. Exclude method-internal docs (BUILD-PLAN, TEST-LOG, build-log, proxies, research) — those have their own maintenance cycles.
...
- **Wrong-home content.** Content that belongs in a different doc — implementation details in UX.md, UX rationale buried in MANIFEST.md, scope discussion in a source-of-truth doc instead of BUILD-PLAN. Name the content and where it should go.
...
2. **Wrong-home content.** For each flagged item: show the content, name the destination, and propose the move. `[PROMPT]` — wait for approval before editing. If approved, edit both source and destination in this step. If the destination is a locked doc (e.g. moving something into BUILD-PLAN during build), flag it for later instead of editing.
