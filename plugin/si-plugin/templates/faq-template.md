...
To record where you'd go if you wanted to change a project doc by hand yourself. That's all it does, and it's less than the question might suggest.

When Claude gives you a link to one of your project docs, that link opens the file in Claude's own viewer — whatever you answered here. What the viewer won't let you do is edit the text. So this setting is about editing, never about reading.

Plenty of people never edit a doc by hand, because Claude does the writing. If that's you, say skip: you still get links, and nothing else changes. It's asked once, during /setup, and never again.

## Why does setup ask whether I work from my computer or my phone?

To set your **working mode** — whether you're usually at your desktop or driving Claude from your phone. It changes one thing: how Claude shows you text that lives in your project files. Set it to **local** (at your computer) and Claude points you to that text with a link, since a file opens instantly for you — it saves re-typing the text into chat. Set it to **remote** (on your phone) and Claude pastes the text into chat as well, because opening a file on a phone means digging through Google Drive, so a link alone would be no use. It's asked once at /setup and defaults to **local** if you skip. You're not locked in: tell Claude "I'm remote today" (or "back at my desk") any time and it switches for that session, reverting afterward. This is the only setting that decides it — the editor question doesn't affect whether you get links.
