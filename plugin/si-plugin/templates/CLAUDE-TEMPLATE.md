...
- **GLOSSARY.md** — the words that have been explained to you, and the names this project has settled on. Claude checks it before using a technical term: a word that's in there has been explained already, so it gets used plainly; a word that isn't gets explained at the point of use and then recorded. So Claude never re-explains what you know, and never assumes you know what it hasn't told you. Entries read the way the term was last explained to you rather than as textbook definitions, and get updated each time it's explained again. Your own chosen names go in too, with your reasoning, and are correct here from then on.
