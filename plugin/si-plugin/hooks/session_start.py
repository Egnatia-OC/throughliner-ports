...
# The plugin's one procedure docset, by directory name under the plugin root.
#
# There were two until 2026-08-08. Docset A (`docs/`) was the heavy docset the
# method was built on, frozen since 2026-08-02 as a known-good fallback for
# Opus 4.8; docset B (`docs-b/`) was authored by subtraction for the 5-series
# and is where the method actually evolved. A was retired on the user's
# explicit call once its three conditions cleared: the blitz branch merged, the
# register-measurement harness consciously waived on the record, and nobody
# left running 4.8.
#
# With one docset there is no choice to make, so the model-detection branch and
# its absent-model fallback were DELETED rather than rewritten — along with the
# `model` payload reader, the recorded-`Model:` reader and the model-family
# regex, which existed only to feed that choice. What replaces the no-strand
# guarantee A used to provide is the missing-folder state line in main(): with
# nothing to fall back to, a missing docset means no procedure docs at all, and
...
def _missing_docset_note(plugin_root):
    """State line for an installed plugin whose procedure docs aren't there.

    This used to be a fallback: with two docsets, a missing `docs-b/` meant
    dropping to docset A, and everything still worked. There is nothing to drop
    to now, so a missing folder means the session has NO procedure docs — the
    skills will name files that don't exist. Saying so plainly is the whole
    remaining protection, and it replaces the no-strand guarantee the frozen
    docset used to carry.

    The distinction that has to survive here: this is about the INSTALLED
    plugin, not the working tree. In the self-hosting dev project the folder can
    be present in `plugin/si-plugin/` and absent from the installed snapshot,
    which is exactly the case that reads as the plugin being broken.
    """
    if not plugin_root or os.path.isdir(os.path.join(plugin_root, _DOCSET)):
        return ""
    return (
        f"[Sovereign Implementer] PROBLEM: the installed plugin has no "
        f"`{_DOCSET}/` folder, so this session has no procedure docs. The "
        "skills will point at files that aren't there, and /plan, /next and "
        "/done can't run properly.\n"
        f"This is about the INSTALLED copy at {plugin_root} — the folder may "
        "well be present in the source you're editing. Reinstalling the plugin "
        "and fully restarting the app is what refreshes the installed copy. "
        "Tell the user plainly rather than working around it."
    )
...
def _behaviour_rules_directive(plugin_root):
    ...
    plus a path in its place. `plugin-behaviour.md` is roughly 50KB, so
    ...
    redirect mechanism was proven in this very payload — back when a docset
    directive shared it, that directive was the one part that reached sessions
    through the truncation, and it was followed.
    ...
    if not plugin_root:
        return ""
    path = "${CLAUDE_PLUGIN_ROOT}/" + _DOCSET + "/plugin-behaviour.md"
    stamp = "carries `docset: B` in its frontmatter"
    return (
...
def main() -> int:
    ...
    # There is one docset, so there is no selection to make and no directive to
    # emit — the skills name `docs-b/` directly. What survives is the check that
    # the folder is actually there in the installed plugin, and it can no longer
    # fall back to anything, so it reports rather than repairs.
    docset_fallback_note = _missing_docset_note(plugin_root)
    ...
    if not has_spec:
        ...
        # /setup is the skill this branch points at, so a missing procedure-doc
        # folder has to be reported to an unadopted folder as well — otherwise
        # the first thing the user runs fails with nothing explaining why.
        if docset_fallback_note:
            msg += "\n\n" + docset_fallback_note

        # The behaviour-rules directive reaches an unadopted folder too, for the
        # same reason.
        #
    ...
    # and build stamp), then the behaviour-rules directive,
    ...
    # everything else — it is the line that can least afford to be cut.
