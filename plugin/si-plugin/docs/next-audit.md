...
Execution procedure for `[audit]`-flavor work lines. Reached from next.md after the run is confirmed and scope is locked. The output contract defines an audit: findings route to the Unprocessed section so /plan can process them into normal work lines — no direct edits to the artifacts the audit reads. What gets read varies — procedure docs, the user's spec, code, UI flows, workflow output, any other artifact; the procedure shape is the same regardless: read many, propose many. An audit line names no files to edit, so its contribution to the run's `Files:` section is nothing; a run of only audit lines has an empty `Files:` list, holding the session to the method docs (QUEUE.md, LOG/, _build.md) — the strictest setting, matching a session that edits no source files.

## If the audit line directs a write into a document, stop and ask [PROMPT]

Before reading, check the line's own wording against the contract. An audit's contract is findings-to-Unprocessed with no direct edits to what it reads. So if the line directs a write into a named document (for example "append findings to MAP.md," or names a findings doc to fill), that contradicts the contract — don't silently follow it. Surface the conflict in plain language and ask which the user wants: file the findings as captures the normal way, and let a later build write the document from the vetted findings; or treat this session as a build that writes the document now. Wait for the answer before reading. The why: a line marked `[audit]` but pointed at a doc-write is a planning slip, and following it silently writes unvetted findings straight into a durable doc — the exact thing the route-to-Unprocessed contract prevents. Exemplar: "This line is an audit, but it says to write findings into review-notes.md. An audit files findings to the queue for vetting first — it doesn't write them into a doc. Want me to file them as captures, or run this as a build that writes review-notes.md directly?"

## Read the target systematically against the criteria [SILENT]

Read every artifact the line names. Apply the criteria pass by pass — one criterion across the whole target, then the next, not mixing criteria per artifact. Don't skim; an audit's value is reading what's there. Accumulate observations in _build.md Changes with precise references (file:line for files) so the user can verify each.
...
Append the approved findings to the Unprocessed section in QUEUE.md, each placed per plugin-behaviour.md Captures placement, each written to the capture-authoring standard (plugin-behaviour.md Captures). Tick each finding in _build.md Progress as `captured` or `dropped`.

## Close [BRIEF, PROMPT]

When the audit line is done, next.md moves to the run's next line. When the whole run is done, tell the user, with the captured/dropped counts. Say: "Run /done to record this and commit, or keep reviewing." Reviewing means re-examining what was already found — not raising new work. Anything new routes through the existing paths: a discovery outside the audit's target follows the discovery rule (plugin-behaviour.md Routing and discipline), and thinking work goes to Unprocessed. No chat summary of the routed findings — the LOG entry /done writes is the single session record.
