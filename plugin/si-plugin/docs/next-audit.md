...
Execution procedure for `[audit]`-flavor work items. Reached from next.md after the run is confirmed and scope is locked. The output contract defines an audit: findings route to the Unprocessed section so /plan can process them into normal work items — no direct edits to the artifacts the audit reads. What gets read varies — procedure docs, the user's spec, code, UI flows, workflow output, any other artifact; the procedure shape is the same regardless: read many, propose many. An audit item names no files to edit, so its contribution to the run's `Files:` section is nothing; a run of only audit items has an empty `Files:` list, holding the session to the method docs (QUEUE.md, LOG/, _build.md) — the strictest setting, matching a session that edits no source files.

## If the audit item directs a write into a document, stop and ask [PROMPT]

Before reading, check the item's own wording against the contract. An audit's contract is findings-to-Unprocessed with no direct edits to what it reads. So if the item directs a write into a named document (for example "append findings to MAP.md," or names a findings doc to fill), that contradicts the contract — don't silently follow it. Surface the conflict in plain language and ask which the user wants: file the findings as captures the normal way, and let a later build write the document from the vetted findings; or treat this session as a build that writes the document now. Wait for the answer before reading. The why: an item marked `[audit]` but pointed at a doc-write is a planning slip, and following it silently writes unvetted findings straight into a durable doc — the exact thing the route-to-Unprocessed contract prevents. Exemplar: "This item is an audit, but it says to write findings into review-notes.md. An audit files findings to the queue for vetting first — it doesn't write them into a doc. Want me to file them as captures, or run this as a build that writes review-notes.md directly?"
