...
There is no test close-out — the test flavor is retired. A check Claude can run is part of building, closed by done-build.md; a check only the user can run is a `[user]` work item, which /next hands over and which never enters a _build.md, so no /done closes it.
