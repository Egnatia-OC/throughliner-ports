...
2. **Self-scope.** Derive the `Files:` list from the run's Claude-work items: read each item's description and rationale, identify the files it will change, and list them. This replaces the old pre-authored Files list — /next reads the work and works out its own scope. If an item's files can't be determined from what it says, that's a signal the item is underspecified; surface it rather than guessing. (`[audit]` items name no files to edit — an audit reads and reports; they contribute nothing to Files. A run of only audit items gets an empty Files list, locking the session to method docs.)
