...
2. **Read the Processed section and find the run.** [SILENT] Read QUEUE.md's Processed section top-down. The **run** is every cleared work item from the top of Processed down to — but never past — the `--- Cleared to run above this line ---` marker. The run **includes** any `[user]` items among the cleared work: /next walks the user through each (Step 3) *without ending the run*. Only the marker bounds the run; a `[user]` item no longer terminates it — this is the whole point of the run being marker-bounded rather than stop-at-first-`[user]`.
   ...
   - **Marker missing.** If Processed contains no `--- Cleared to run above this line ---` marker at all, treat that exactly like nothing cleared — a missing marker means *nothing* is cleared, never everything. The run is defined as everything above the marker, and with no marker that range has no terminator; reading it as "all of Processed" would hand an unattended run work nobody greenlit, including work deliberately held below the line. Soft-stop [BRIEF]: tell the user the queue's cleared-to-run marker is missing, recommend /plan to restore it and re-confirm what is actually cleared, and stop. Don't build anything.
   ...
   **Never check whether a `[user]` item is already done — not up front, not in passing, not as a trailing note.** A `[user]` item is walked through, and that is all: its whole lifecycle carries no completion ask anywhere (plugin-behaviour.md, "There is no completion ask"). The why: any ask, in any position, treats ready work as probably-already-done and makes the user re-assert it before Claude will help — the trailing-note position was the last surviving ask and it is removed too, because every surviving position becomes the place that pressure re-enters. If the user has already done one, they'll say so, and that's the moment it gets recorded (recommend /done); if they say nothing, walk it through.
...
4. **Remove the run's Claude-work items from QUEUE.md** now that _build.md holds them (sub-step 3) — the queue is free for other sessions. **The run's `[user]` items are *not* removed — they stay in QUEUE.md.** A `[user]` item is walked through in Step 3, not built, and it's closed (logged and removed) only later by /done or /plan once the user has done it; extracting it into _build.md would strand it, since _build.md is deleted at close. So only the Claude-work items move. This removal is deliberately destination-first: those items were written into _build.md *before* being removed here, so the run is never lost even if something interrupts between the two. There's no both-sections window to avoid at this step — the destination is the working file (_build.md), not the other QUEUE section — so the source-first ordering the keep-step uses (plan.md) doesn't apply here; destination-first is the safe order when moving out of QUEUE into the working file. /done deletes _build.md after close. **This removal is a run's whole write access to the Processed section: a run removes items from Processed and never inserts them.** New work discovered mid-run is captured to Unprocessed (Step 2's discovery rule); moving anything *into* Processed is processing, which belongs to /plan because that decision is the user's. The queue lint backstops this — a work-item heading inserted under Processed while a build is active gets flagged.
...
**No completion ask, anywhere in this branch — not leading, not trailing.** Don't open with "have you already done this one?", and don't close a walk-through with "…or tell me if you've already done it and I'll just record it" — the trailing-note form is an ask too, and it is removed along with every other position the ask used to occupy (plugin-behaviour.md, "There is no completion ask"). The why: an ask in any position treats ready work as probably-already-done and makes the user re-assert it before Claude will help, and each surviving position becomes the place that pressure re-enters. A `[user]` item is walked through, full stop. If the user volunteers that it's already done, take them at their word: don't walk it through — it's completed work to record, so recommend /done. If they say nothing, walk it through.
