...
2. **Read the Processed section and find the run.** [SILENT] Read QUEUE.md's Processed section top-down. The **run** is every cleared work item from the top of Processed down to — but never past — the `--- Cleared to run above this line ---` marker. The run **includes** any `[user]` items among the cleared work: /next hands each over (Step 3) *without ending the run*. Only the marker bounds the run; a `[user]` item no longer terminates it — this is the whole point of the run being marker-bounded rather than stop-at-first-`[user]`.
   ...
   - **All cleared work is handovers.** If the run holds no Claude-work items — every cleared item is a `[user]` handover — there's nothing to build. Skip Step 2's build scaffolding and go straight to Step 3's handover branch [PROMPT].

   **The already-done check rides the handover; it never leads.** A `[user]` item can already have been done — the user ran it after a past session handed it over, but nothing removed it from Processed, so it's still sitting there. That real case is handled, but *not* by asking up front: the handover branch (Step 3) leads with the walk-through and carries the already-done check as a light trailing note, only on an item that could plausibly have been completed before. Do **not** open a handover with "have you already done this one?" — leading with it inverts the handover into a probably-already-done assumption the user must push back on. Detection stays by asking, not by scanning the filesystem (a handover can be a device check or a decision, with no artifact to find); what changes is *where* the ask sits — trailing, not leading (Step 3 handover branch).
...
**Lead with the walk-through, not with "have you done this?".** The handover's whole value is that Claude helps the user do the work — so the message *leads* with the walk-through offer, never with the completion-ask. Open by naming what's theirs to do, that Claude will run whatever parts it can and guide the rest, then walk them through the described steps the item carries (a `[user]` line records the concrete steps — plugin-behaviour.md Captures). Do **not** open with "have you already done this one?" — leading with the completion-ask inverts the handover, treating a ready piece of work as probably-already-done and making the user re-assert it before Claude will help. That inversion is the bug this branch was rewritten to remove.

**Where the already-done check goes instead — a trailing note, only where it applies.** The completion-ask still has a real job: a `[user]` item can already have been done in a past session that never recorded it (Step 1). But it rides at the *end* of the handover, as a light note — "…or let me know if you've already done this and I'll just record it" — and only on an item that *could* plausibly have been completed before (one a past /next handed over). A freshly-cleared item that has never been handed over carries no such note at all. If the user says it's already done, don't walk it through: it's a completed handover to record, so recommend /done. The note never pre-empts the walk-through; the walk-through always leads.

Hand it over **collaboratively — not as a bare hand-off.** State plainly what the user needs to do and why it's theirs to run, then guide them through it: run whatever parts Claude *can* (commands, setup, anything Claude can drive), explain in plain words what the user needs to check or do, and walk them through it step by step. The `[user]` tag marks who ultimately performs or witnesses the step — it does not mean Claude steps back and leaves the user to it.

**Multiple independent handovers are presented together, not one at a time.** When the final pass leaves more than one `[user]` item and they're *independent* of each other, present them all in one message — each naming its own close (run /done, or raise at the next /plan) — rather than releasing them one per message. This is the [SEQUENCE] inversion in plugin-behaviour.md for "a deterministic result set produced under criteria the user already approved": every one of these handovers was cleared in /plan and sits above the line, so they're an approved, independent set, and seeing them together lets the user plan and batch them instead of being drip-fed. The trailing already-done note (above) attaches to this bundle where it applies — one closing note covering any of the bundled items that could have been done before, never a per-item completion-ask ahead of each. **Carve-out:** if two handovers have a real order or dependency between them, sequence *those* — the bundle is for independent handovers only.
