...
Between build items, keep going autonomously — the user confirmed the whole run at Ready, so there's no per-item re-confirmation. Tick each item in _build.md Progress as it finishes before starting the next.
