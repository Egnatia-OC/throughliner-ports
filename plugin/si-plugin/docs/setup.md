...
**The plugin-wide behaviour rules govern this skill as they govern every other one, and they are read here like any other session's.** This corrects a claim this doc used to make — that the rules "aren't loaded yet" because /setup runs before a project is adopted. That described the older delivery model, in which the session-start hook inlined the rules into an adopted project's context, and it is no longer true of the hook that runs underneath this docset: the hook now emits a directive telling the session to *read* `plugin-behaviour.md` from the installed plugin, and that file ships with the plugin, so it is readable whether or not any folder has been adopted. The rules were absent from /setup not because they were unavailable but because the unadopted branch did not point at them, and it now does. The correction matters because the ungoverned window it left open was the moment a folder is adopted and files are created — the highest-consequence moment in the whole method, and the only one a brand-new user ever sees, so it is precisely the moment at which the communication and safety rules are most needed rather than least.

This doc nonetheless carries no response-shape tags (the bracketed `[BRIEF]`/`[SEQUENCE]`-style markers other procedure docs use), and none should be added: the prose in each step already carries the behaviour directly — when to stop and wait for the user, when to keep output short, one question per message — and that arrangement is deliberate rather than a consequence of the rules being absent.

**/setup's own local rules stay in force and are not made redundant by inheriting the behaviour rules.** Leave the user's content untouched, never overwrite anything, never rename a file blindly, and use the user's own words verbatim. The tempting reading — that inheritance now covers these, so they can be deleted as duplication — is the wrong one. These rules are specific to adopting a stranger's folder, where the cost of a mistake is the user's own material, and belt-and-braces is the correct posture at the one moment in the method where files are created rather than edited.
