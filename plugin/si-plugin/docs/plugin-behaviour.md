...
- Memory boundaries — the project's records belong in the project's docs, never in Claude's memory. Route them there: ideas and discoveries to Captures, design decisions to QUEUE.md and SPEC.md, project state to the method docs. The why: memory doesn't travel with the project and the user can't read it, so a project record saved to memory is a record the project has lost — the method docs are the project's record. Memory stays free, and is the right home, for what no project doc owns: user preferences, working style, communication feedback, cross-project facts. Scope: every session type, any moment something looks worth remembering — the test is whether a project doc owns it.
- Doc routing: SPEC.md = what/who/how/why the project exists. QUEUE.md = what to work on next. LOG/ = what happened.
- /plan is for planning, /next is for building. Don't cross them.
- No planning work in any execution skill. Planning work is processing captures — routing them: promoting, parking, dropping, deciding their fate. Making a capture and approving its wording stays open to every session type; processing captures happens only in /plan. The why: capture-processing is where design decisions the user owns get made, so an execution skill that quietly processed one would settle an owner's decision off the user's radar — the exact thing unattended execution must never do. Scope: every execution session type — /next build, test, audit, freeform, and any execution type added later. For test sessions specifically: when the user runs a test and judges its outcome, that judging is the test work itself, not planning; filing the outcome is capture-making, which execution skills do.
- When you notice something mid-session that isn't in the active batch's described work, decide by one rule: is it needed to complete the batch's entries? Needed and minor (a small prerequisite) → ask to add it; needed and significant (multiple files, design uncertainty) → propose splitting; not needed → capture it and continue; the batch's premise is broken → halt and course-correct. "Not needed → capture and continue" is the common case, and it's the defined routing move: route the discovery to Captures at the moment of noticing, using the same flow as a user-raised item — draft the wording, show it, ask "anything else?", then resume. Don't hold it in conversation to deal with later; an unrouted discovery survives only in memory, the failure a short session can't recover from.
...
- **Staleness watch.** When batches or captures sit long enough that surrounding code or rules have moved past them, Claude flags them for review (drop / rewrite / keep). Same surfacing moments as unpark watch. The fix path splits on whether the flag is a fate decision or a moved pointer. The drop / rewrite / keep review is a fate decision about whether the item is still valid, and fate decisions are /plan's, always — flag and defer. But a drifted pointer on an otherwise-valid item — a line or location reference whose *target content is unchanged* — is not a fate decision; it's mechanical maintenance, and may be corrected at the flagging moment with the user's approval, riding the current commit, the same ride-along as hash backfills and rolled-in user edits. A content change in the referenced material (the quoted wording itself moved, which may signal the item's premise shifted) is not mechanical and goes to /plan. The why: the fate decision needs planning, a moved pointer doesn't. Scope: this fix-at-close path is for skills that commit (/done and /plan); /next's pre-flight stale scan stays surface-and-recommend-/plan, since /next doesn't edit the queue's other batches.
