...
## Learnings from the second run (2026-08-05, branch `overnight-blitz-2026-08-05b`) — further amendments

1. **The capture target is retired, not just softened.** Two runs, two healthy sweeps: 5 findings, then 2, against a "~10–15" target. A number reads as a quota even with the ceiling-of-honesty amendment sitting next to it, so Phase 2's heading no longer carries one. Report what the sweeps honestly find; a clean sweep is the finding.
2. **Hold reasons go in the blitz-close LOG entry** — folded into Phase 2 above. The first run put triage notes in the morning handoff message, which scrolls away; the committed entry gives the next /plan the triage pre-done and durable.
3. **A stated interim fix inside a bigger undesigned item does not pass the bar.** Building the fragment can't remove the item, so it leaves the docs changed and the queue item still standing — drift by construction. Leave the item whole and name the ready fragment in the closing advisory instead. (Hit live on the stale-viewer mitigation.)
4. **`--move-section` into Processed lands at BOTTOM — below the readiness line.** Bar-passing items moved into Processed need `--position`, or a follow-up reorder with `--marker-after`, or the file reads as though nothing was cleared. Cost one extra full reorder this run.
5. **Delete `_build.md` before writing the commit-message file.** The scope-lock correctly denies `COMMIT_MSG.tmp` while a build is active; the close's delete-then-commit order is load-bearing, not cosmetic. The denial is the hooks working — don't read it as a fault.
6. **Order corpus-wide mechanical passes against targeted edits.** A bulk find-replace script changed text that later prepared edits exact-matched on, and an edit failed mid-run. Run bulk passes before drafting targeted edits to the same files, or re-grep the exact text immediately before each edit.
