...
class TestV74SessionOpenStatus:
    ...
    def test_batch_counts_folder_mode(self, adopted_folder):
        """Folder mode: 1 active, 2 queued, 1 parked (shipped excluded)."""
        code, parsed, _ = run_hook(
            "session_start.py", {"cwd": str(adopted_folder)}
        )
        ctx = parsed["hookSpecificOutput"]["additionalContext"]
        assert "1 active" in ctx
        assert "2 queued" in ctx
    ...
    def test_top_queued_batches_shown(self, adopted_folder):
        """V78: queued batches appear in Coming up section."""
        code, parsed, _ = run_hook(
            "session_start.py", {"cwd": str(adopted_folder)}
        )
        ctx = parsed["hookSpecificOutput"]["additionalContext"]
        assert "Coming up:" in ctx
        assert "Add notifications" in ctx
        assert "Export data" in ctx

    def test_top_queued_with_active_shows_all(self, adopted_folder):
        """When active batch exists, Coming up shows all queued batches."""
        code, parsed, _ = run_hook(
            "session_start.py", {"cwd": str(adopted_folder)}
        )
        ctx = parsed["hookSpecificOutput"]["additionalContext"]
    ...
    def test_top_queued_single_file_mode(self, adopted_single_file):
        """Single-file mode with 1 queued batch: no Coming up section."""
        code, parsed, _ = run_hook(
            "session_start.py", {"cwd": str(adopted_single_file)}
        )
        ctx = parsed["hookSpecificOutput"]["additionalContext"]
